...
function loadStatesNew(c_el, state_val,s_el,city_val) {
    cid = parseInt(c_el.value);
    if (isNaN(cid) || cid < 1)
        return false;
    //alert(generateUrl('common', 'loadDropDown', [cid, 'states'], webroot));
...
function loadCities(s_el, city_val) {
    sid = parseInt(s_el.value);
    if (isNaN(sid) || sid < 1)
        return false;
    //alert(generateUrl('common', 'loadDropDown', [cid, 'states'], webroot));
    callAjax(generateUrl('common', 'loadDropDown', [sid, 'cities'], webroot), 'a=1', function (t) {
        var ans = parseJsonData(t);
        if (ans === false) {
            return false
        }
        ...
        if (target_el.length < 1)
            target_el = $(s_el).parents('form:first').find('select[name="ua_city"]');
        target_el.html('');
        target_el.append('<option value="">City Town</option>');
        $.each(ans, function (i, value) {
            target_el.append('<option value="' + i + '">' + value + '</option>');
        });
