...
class UsersController extends BackendController {
	...
	function addUserForm($user_id) {
			...
			$frm->addRequiredField('Username', 'user_username', null, 'user_username');
			$frm->addRequiredField('Email Address', 'user_email', null, 'user_email');
	...
	function update_user_cancellation_request_status(){
		...
		if ($mod=='approve'){
				//$arr_pro_com=(array)Settings::getSetting("CONF_PROCESSING_ORDER_STATUS");
				/*echo '<pre>';
				print_r($arr_pro_com);
				echo '</pre>';*/
	...
	function update_seller_cancellation_request_status(){
		if ($this->canviewcancellationrequests != true) {
            $this->notAuthorized();
        }
		$post = Syspage::getPostedVar();
		$mod = $post['mod'];
        $id = intval($post['id']);
		$commission_cancel_reason = $post['commission_cancel_reaason'];
		...
		if ($mod=='approve'){
				...
				if (in_array($cancellation_request["opr_status"],$arr_pro_com)){	
					$oObj=new Orders();
					$oObj->addChildOrderHistory($cancellation_request["opr_id"],Settings::getSetting("CONF_DEFAULT_CANCEL_ORDER_STATUS"),Utilities::getLabel('M_Your_Cancellation_Request_Approved'),true,"",0,$commission_cancel_reason,$cancellation_request["cancellation_request_message"]);
				}else{
					//echo 'here';
					//exit;
					Message::addError('This order is not eligible for cancellation.');
        ...
        switch($mod) {
            case 'approve':
            	$data_to_update = array(
					'cancellation_request_status'=>3,
					'cancellation_charge_type'=>$commission_cancel_reason,
	            );
				Message::addMessage('Cancellation request has been approved successfully.');
            break;
            case 'decline':
    	        $data_to_update = array(
					'cancellation_request_status'=>2,
					'cancellation_request_decline_reason'=>$commission_cancel_reason,
            	);
				Message::addMessage('Cancellation request has been declined successfully.');
            break;
        }
		if($crObj->updateCancellationRequestStatus($id,$data_to_update)){
			$emailNotificationObj=new Emailnotifications();
	  		$emailNotificationObj->SendSellerCancellationRequestUpdateNotification($id);
			$arr = array('status'=>1, 'msg'=>Message::getHtml());
			die(convertToJson($arr));
		}else{
			Message::addErrorMessage($crObj->getError());
