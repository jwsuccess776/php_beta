...
class ReviewsController extends BackendController {
	...
	function user_reviews($myaction="",$val="",$seller_id="") {
		if ($this->canview != true) {
            $this->notAuthorized();
        }
        
        if($myaction=='update' && $val!="" && $seller_id!="")
        {
			$mydb = Syspage::getdb();
			$query="UPDATE tbl_order_rating SET tor_status='$val' WHERE tor_id='$seller_id'";
			$mydb->query($query);
			Message::addMessage('Rating status updated successfully.');	
    ...
    function view_user_reviews($seller_id="",$myaction="",$val="")
    {
		if ($this->canview != true) {
            $this->notAuthorized();
        }
        ...
        {
			$mydb = Syspage::getdb();
			$query="UPDATE tbl_users SET user_status='$val' WHERE user_id='$seller_id'";
			$mydb->query($query);
			Message::addMessage('User status updated successfully.');	
			Utilities::redirectUser(Utilities::generateUrl('reviews', 'user_reviews'));
		}
