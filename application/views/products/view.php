...
{ event: "viewItem", item: "<?php echo 'sellsa-'.$product_info['prod_id']?>" }
...
<script>
  fbq('track', 'ViewContent', {
    currency: 'zar',
    content_ids: '<?php echo 'sellsa-'.$product_info['prod_id']?>',
...
<div>
    <div class="body clearfix">
        <div class="breadcrumb">
            <div class="fixed-container">
                <ul>
                    ...
                    <?php
                    foreach ($product_info["product_category"] as $pcat) {
                        ?>
        ...
        <div itemscope itemtype="http://schema.org/Product">
            <div class="fixed-container">
                                ...
                                <div id="product-gallery" class="eagle-gallery img400">
                                    <div class="owl-carousel"> 
                                        <?php foreach ($product_info["images"] as $iKey => $iVal): 
                                            if (strpos($iVal["image_file"], 'https') !== false || strpos($iVal["image_file"], 'http') !== false) { ?>
                                                  <img src="<?php echo $iVal['image_file'] ?>" data-medium-img="<?php echo $iVal['image_file'] ?>" data-big-img="<?php echo $iVal['image_file'] ?>" data-title="" alt="<?php echo $iVal['prod_name'] ?>">
                                            <?php }else{?>
                                ...
                                <div class="review-side">
                                    ...
                                    <div class="review-write"> 
                                        ...
                                        <a class="logged_in" href="<?php echo Utilities::generateUrl('products', 'send_message', array($product_info["prod_id"])) ?>">
                                            ...
                                            <?php echo Utilities::getLabel('L_Ask_a_Question') ?></a> <a class="<?php
                                        if ($product_info["favorite"]): echo 'active';
                                        endif;
                                        ?> itemfav" id="item_<?php echo $product_info["prod_id"]; ?>" href="#">
                            ...
                            <div class="selecting-things clearfix">
                                ...
                                <div class="options">
                                    ...
                                    <ul class="list-unstyled">
                                        <?php
                                        if ($product_info["prod_brand"]) {
                                            ?>
                                            <li><?php echo Utilities::getLabel('L_Brand') ?>: 
                                                <?php
                                                if ($product_info["brand_status"]) {
                                                    ?>
                                    ...
                                    <div id="product">
                                        <?php
                                        if (count($product_info["addon_products"]) > 0) {
                                            ?>  
                                            <div class="cart-box">
                                                ...
                                                <table class="cart-tbl">
                                                    ...
                                                    <?php
                                                    foreach ($product_info["addon_products"] as $product) {
                                                        ?>
                                                        <tr>
                ...
                <div class="description-tabs">
                    <ul class="tabs">
                        ...
                        <?php
                        if ($product_info["attribute_groups"]) {
                            ?><li><a href="#tab4" class="ripple"><?php echo Utilities::getLabel('L_Specifications') ?></a></li><? } ?>
                                <?php
                                if (($product_info["totReviews"] > 0) && Settings::getSetting("CONF_ALLOW_REVIEWS")) {
                                    ?><li><a href="#tab2" class="ripple"><?php echo Utilities::getLabel('L_Reviews') ?></a></li>
                                <? } ?>
                    ...
                    <!--                                                        <td><em><strong><?php echo $val["country_name"] != "" ? $val["country_name"] : Utilities::getLabel('L_Everywhere_else') ?></strong></em> <?php echo Utilities::getLabel('L_by') ?> <strong><?php echo $val["scompany_name"] ?></strong> <?php echo Utilities::getLabel('L_in') ?> <?php echo $val["sduration_label"] ?></td>-->
                                                    <td>
                                                        <em>
                                                            <strong><?php echo empty($val['city_name']) ? '' : $val['city_name']; ?> <?php echo $val["pship_state"] != "-1" ? $val["state_name"] : Utilities::getLabel('L_Everywhere_else') ?></strong>
                                                        </em> 
                                                        <?php echo Utilities::getLabel('L_by') ?> <strong><?php echo $val["scompany_name"] ?></strong> 
                                                        <?php echo Utilities::getLabel('L_in') ?> <?php echo $val["sduration_label"] ?>
                                                    </td>
                                    ...
                                    <?php
                                        // calc width
                                        $payment_count = 1;
                                        if (!empty($custom_payment_option)) {
                                        ...
                                        $width = 100 / $payment_count;
                                    ?>    
                                    <!--Added on 17-02-2020-->
                                    <div class="gap"></div>
                                    <table class="specTable">
                                        <tr>
                                            <th colspan="<?php echo $payment_count;?>" class="groupHead"><?php echo $product_info["shop_name"] . " " . Utilities::getLabel('L_ACCEPTS_PAYMENTS_FROM') ?></th>
                                        </tr>
                                        <tr>
                                            <td style="text-align:center; width:<?php echo $width;?>%">
                                                <img src="https://sidpayment.com/wp-content/uploads/2017/03/sid-instant-eft-logo2x.png" width="75px"/>
                                            </td>
                                            <?php
                                            if (!empty($custom_payment_option)) {
                                                foreach ($custom_payment_option as $key => $val) {
                                                    if (!empty($val['status'])) {
                                                        ?>
                                                        <td style="text-align:center; width:<?php echo $width;?>%">
                                                            <img src="<?php echo $val['image_url']; ?>" width="75px"/>
                                                        </td>
                                                        <?php
                                                    }
                                                }
                            ...
                            if ($product_info["related"]) {
                                ...
                                <div class="section category-list" >
                                    ...
                                    <?php
                                    foreach ($product_info["related"] as $product) {
                        ...
                        <?php
                        if ($product_info["smart_recommended_products"]) {
                            ?>
                            <div class="section category-list" >
                                <div class="main-heading">
                                ...
                                <?php
                                foreach ($product_info["smart_recommended_products"] as $product) {
                                    include CONF_THEME_PATH . 'common/product_thumb_view.php';
                                }
                                ?>
                            </div>
                    ...
                    <?php
                    if ($product_info["also_bought_products"]) {
                        ?>
                        <div class="section category-list" >
                            <div class="main-heading">
                            ...
                            <div class="shop-list products-carousel <? if (count($product_info["also_bought_products"])<6):?>less_items<?php endif; ?>">
                                 <?php
                                 foreach ($product_info["also_bought_products"] as $product) {
                                     include CONF_THEME_PATH . 'common/product_thumb_view.php';
                                 }
                                 ?>
                        </div>
            ...
            <?php
            if (Settings::getSetting("CONF_USE_SSL") == 1) {
                ?>
...
<script> gtag('event', 'page_view', 
            {'send_to': 'AW-849203835', 'ecomm_pagetype': 'product', 'ecomm_prodid': '<?php echo'sellsa-' . $product_info['prod_id'];?>','ecomm_totalvalue':'<?php echo $product_info['prod_sale_price'].' ZAR'?>','user_id': '<?php echo $email;?>'});</script>
