...
function listPaging(page) {
	var frm = document.search_filters;
    frm.pg.value = parseInt(page);
    loadProducts();
}
...
function loadProducts(){
	...
	serializedStr="";
	var frm = document.primarySearchForm;
	var serializedData = remove_duplicates($("#search-filters").serializeArray());
	jQuery.each( serializedData, function( i, field ) {
	  if(field.value){
      		serializedStr = serializedStr.concat(field.name+'='+field.value + "&") ;
	  }
    });
	serializedStr = serializedStr.substring(0, serializedStr.length - 1);
    var filters = cleanEmptyValues(serializedStr);
    if (frm.sort.value != "") {
	...
	window.location.href = push_string;
    //filterProducts(document.primarySearchForm);
}
...
function searchStore(ev){
	if(ev.which == 13)
	{
		var searchKey = document.getElementById("searchKeyword").value;
		event.preventDefault();
		document.getElementById("shop_keyword").value = searchKey;
		loadProducts();
	}
}
