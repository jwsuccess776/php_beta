                ...
                <?php if ($total_records > 0): ?>
                        <div class="darkgray-form">
                            <div class="tabs-form">
                                ...
                                <?php echo str_replace("<br>", " ", $frmnew->getFormHtml()); ?>
                            </div>
                        </div>
                        <div class="tbl-listing">
                            ...
                            <table>
                                <tbody>
                                    ...
                                    <?php
                                    $cnt = 0;
                                    foreach ($arr_listing as $sn => $row): $sn++;
                                        ?>
                                        <tr>
                                            <td><span class="cellcaption"><?php echo Utilities::getLabel('L_Inv_Number_Date') ?></span> <span class="blue-txt"><strong>
                                                        ...
                                                        <a href="<?php echo Utilities::generateUrl('account', 'view_order', array($row["order_id"], $row["opr_id"])) ?>" class="order">
                                                            ...
                                                            if (!($row["totOrders"] > 1))
                                                            {
                                                                echo $row["order_invoice_number"];
                                                            }
                                                            else
                                                            {
                                                                echo $row["opr_order_invoice_number"];
                                                            }
                                                            ?></a>
                                            ...
                                            <td nowrap="nowrap"><span class="cellcaption"><?php echo Utilities::getLabel('L_Action') ?></span>
                                                <?php
                                                if ($row["opr_status"] == Settings::getSetting("CONF_RETURN_REQUEST_ORDER_STATUS"))
                                                {
                                                    ?>
                                                ...
                                                {
                                                    echo $row["orders_status_name"];
                                                }
                                                ?>

                                                <?php
                                                if ($row["orders_status_name"] == 'Payment Pending')
                                                {
                                                    $Date = $row["order_date_added"];
                                                    $expireDate = date('M d, Y H:i:s', strtotime($Date . ' + ' . AUTO_CANCELLED_DAYS . 'days'));
                                                    ?>
                                                    <!--                                                    
                                                    <a target="_blank" href="<?php //echo Utilities::generateUrl('account', 'dashboard_buyer', array('view_payment_option', $row["order_id"]))    ?>"  style="display: block">
                                                                                                            View Payment Option
                                                                                                        </a>
                                                    -->
                                                    <a target="_blank" style="display:block;" href="<?php echo Utilities::generateUrl('account', 'dashboard_buyer', array('pay_now', $row["order_id"], $row["opr_id"])) ?>">Pay Now</a> 
                                            ...
                                            <td nowrap="nowrap">
                                                ...
                                                <?php if (Settings::getSetting("CONF_ALLOW_REVIEWS")): ?>  
                                                    <?php
                                                    if (($row["opr_status"] == 7) || ($row["opr_status"] == 13) || ($row["opr_status"] == 5) || ($row["opr_status"] == 6) || ($row["opr_status"] == 9))
                                                    {
                                                        ?>	
                                                    ...
                                                    {
                                                        ?>	 
                                                        <a class="actions disabled" title="<?php echo Utilities::getLabel('L_Feedback') ?>" href="<?php echo Utilities::generateUrl('account', 'feedback', array($row["opr_id"])) ?>"><img alt="" src="<?php echo CONF_WEBROOT_URL ?>images/retina/list.svg"></a>
                                                        <?php
                                                    }
                                                endif;
                                                ?>
