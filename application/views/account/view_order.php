        ...
        <div class="data-side">
          ...
          <div class="box-head no-print">
            ...
            <div class="padding20 fr">  
            	<a href="<?php echo Utilities::generateUrl('account', 'orders')?>" class="btn small ">&laquo;&laquo; <?php echo Utilities::getLabel('L_Back_to_orders')?></a> 
            	
            	<a target="_blank" href="<?php echo Utilities::generateUrl('account', 'print_order',array($order_detail["order_id"],($child_order || $order_detail["primary_order"]!=1)?$order_detail["opr_id"]:''))?>" class="btn small secondary-btn "><?php echo Utilities::getLabel('L_Print_Order')?></a> 
            
            	
            		<a href="<?php echo Utilities::generateUrl('account', 'send_message_to_buyer',array($order_detail['shop_user_id'],$order_detail['opr_id'],$order_detail['order_invoice_number']))?>" class="btn small ">Message Seller</a>
          ...
          <div class="space-lft-right">
            ...
            <div class="areaaddress">
			 ...
			 <?php echo '<strong>'.$order_detail["order_billing_name"].'</strong><br/>'.((strlen($order_detail['order_billing_address1']) > 0)?$order_detail['order_billing_address1']:'') .((strlen($order_detail['order_billing_address2']) > 0)?'<br/>'.$order_detail['order_billing_address2']:'') . ((strlen($order_detail['order_billing_city']) > 0)?'<br/>'.$order_detail['order_billing_city'] . ', ':'') . $order_detail['order_billing_postcode']; ?><br><?php echo $order_detail['order_billing_state']; ?>, <?php echo $order_detail['order_billing_country'].((strlen($order_detail['order_billing_phone']) > 0)?'<br/><strong>T</strong>:'.$order_detail['order_billing_phone']:'').((strlen($order_detail['order_billing_fax']) > 0)?'<br/><strong>F</strong>:'.$order_detail['order_billing_fax']:''); ?>
              ...
              <?php 
			  //die($order_detail['opr_shipping_required']."#");
			  if (($order_detail['opr_shipping_required'] && $child_order>0) || ($order_detail['order_shipping_required'] && $child_order==null)) {?>
              <aside class="grid_2">
            ...
            <table class=" tbl-normal">
              <tbody>
                ...
                <tr>
                  ...
                  <a target="_blank" href="<?php echo Utilities::generateUrl(preg_replace('/[^A-Za-z0-9-]+/', '-', $row["opr_name"]),'products',array('view',$row["opr_product_id"]))?>">              
                  <span class="item_Title"><?php echo $row["opr_name"]?><?php echo  $customization_cost;?></span>
                  </a>					<?php foreach ($row['order_options'] as $option) { ?>
               					 <br />
