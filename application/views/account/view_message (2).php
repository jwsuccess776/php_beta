          ...
          <table>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                  		<span class="cellcaption"><?php if ($thread["thread_type"]=="O") {
							echo Utilities::getLabel('L_Order'); 
						}  elseif ($thread["thread_type"]=="S"){ 
							echo Utilities::getLabel('L_Shop'); 
						} else { echo Utilities::getLabel('L_Product');
                      	...
                      	<?php if ($thread["thread_started_by"]==$user_details["user_id"]) : ?>
	                        <?php 
	                         ...
	                         echo '<a href="'.Utilities::generateUrl('account','view_order',array($thread["opr_order_id"],$thread["opr_id"])).'">'.$thread["opr_order_invoice_number"].'</a>';
                        ...
                        <?php else:?>
                            ...
                            //echo $thread["opr_order_invoice_number"];
			echo '<a href="'.Utilities::generateUrl('account','sales_view_order',array($thread["thread_record"])).'">'.$thread["opr_order_invoice_number"].'</a>';
                        ...
                        <?php else :?>
 <a target="_blank" href="<?php echo Utilities::generateUrl(preg_replace('/[^A-Za-z0-9-]+/', '-', thread["prod_name"]),'products',array('view',$thread["prod_id"]))?>"><?php echo $thread["prod_name"]?></a>
 						<?php endif;?>
                  </td>
