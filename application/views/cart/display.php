...
foreach($products as $type){
        $grouped_types[$type['shop_id']][] = $type;
        $icount++;
}


$products=$grouped_types;
	//print_r($products); 
  /* foreach ($grouped_types as $product) { 
    print_r($product);   
   }*/
   ...
   foreach ($products as $p=>$k) { 
      foreach ($k as $key => $value) {
        $p_id = 'sellsa-'.$value['product_id'];
        $cart_itm_gtag[] = array(
        'id'=>$p_id,
        'google_business_vertical'=>'retail'
        );
        
        //$p_id = 'sellsa-'.$product[0]['product_id'];
        $cart_itm[] = array(
      'id'=>$p_id,
 ...
 /* echo "<pre>";
  ...
  //$cart_itm = array(); 
   ...
   foreach ($products as $product) { 
    ...
    // $cart_itm[] = array(
    //   'id'=>$p_id,
    //   'price'=>number_format((float)$product[0]['price'], 2, '.', ''),
...
{ event: "viewBasket", item: <?php echo json_encode($cart_itm); ?>}
...
<script>
  gtag('event', 'page_view', {
    ...
    'ecomm_prodid': '<?php echo json_encode($cart_itm_gtag); ?>',
...
?>
		<table class="cart-tbl" width="100%" border="0" cellspacing="0" cellpadding="0" >
            <thead>
              <tr>
                <th width="20%"><?php echo Utilities::getLabel('M_SELLER_INFORMATION')?></th>
...
<?php 	$_cnt=0;	
 		foreach($prod as $product)
 		{
			...
			$_product_shop = $this->Shops->getData($product['shop_id']);
...
<?php if($_cnt=='1') { ?>
 ...
 <div class="cart-bottom">
                <ul>
                  <li>
                  		<strong class="txt-12"><?php echo Utilities::getLabel('L_Sold_By') ?> : </strong><br>
                  		<strong  class="blue-txt"><a style="color: #ff3a59" href="<?php echo Utilities::generateUrl('shops', 'view',array($product['shop_id'])) ?>"><?php echo $product["seller_address"]["shop_name"]?></a> </strong> 
                  </li>
                   
                  <li> <!--?xml version="1.0" encoding="utf-8"?--> 
	                 <?php  if (Settings::getSetting("CONF_ALLOW_REVIEWS")){?> 
                   	 <ul class="rating" style="display: block">
                      <?php for($j=1;$j<=5;$j++){ ?>	
                  		<li class="<?php echo $j<=round($_product_shop["shop_rating"])?"active":"in-active" ?>"><svg xml:space="preserve" enable-background="new 0 0 70 70" viewBox="0 0 70 70" height="18px" width="18px" y="0px" x="0px" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" id="Layer_1" version="1.1">
                        <g><path d="M51,42l5.6,24.6L35,53.6l-21.6,13L19,42L0,25.4l25.1-2.2L35,0l9.9,23.2L70,25.4L51,42z M51,42" fill="<?php echo $j<=round($_product_shop["shop_rating"])?"#ff3a59":"#474747" ?>" /></g>
                    	</svg></li>
                  		<?php } ?>  
                    </ul>
<?php  } //end if ?>
                    ...
                    <strong> <?php echo Utilities::getLabel('L_Location') ?>:</strong> <br/>
                    <?php echo $_product_shop["state_name"]?>, <?php echo $_product_shop["country_name"]?>	
                  </li>
                  
                  <br/>
                  <?php  if (Settings::getSetting("CONF_ALLOW_REVIEWS")):?> 
                  <li style="display: inline-block;text-align: center;width: 48%">
                  	<strong class="txt-15"><?php echo $_product_shop["totReviews"]?></strong><br>
                    <?php echo Utilities::getLabel('L_Reviews') ?>
                  </li>
                  
                  <?php  endif; ?>  
                  <li style="display: inline-block;text-align: center;width: 48%">
                  	<strong class="txt-15"><?php echo $_product_shop["totProducts"]?></strong><br>
...
<?php } ?>              
                ...
                <td><span class="mobile-caption"><?php echo Utilities::getLabel('M_Image')?></span>
                  <div class="pro-image">

                  <a href="<?php echo Utilities::generateUrl('products','view',array($product["product_id"]))?>">
                    <?php
                      if($product['imported_image']!=''){
                          $b= $product['imported_image'];
                      }else{
                          $b = Utilities::generateUrl('image','product_image',array($product["product_id"],'THUMB'));
                      }
                    ?>
                  <img src="<?php echo $b; ?>" alt="<?php echo $product["name"]?>"/> 
...
?>
 ...
 <div class="total-bottom clearfix" style="margin-bottom: 10px !important">
              <div class="btn-row"> <a href="<?php echo Utilities::getSiteUrl()?>" class="btn secondary-btn"><?php echo Utilities::getLabel('M_Continue_Shopping')?></a>  <a href="<?php echo Utilities::generateUrl('cart','checkout')?>?shop=<?=$product['shop_id']?>" class="btn primary-btn "> <?php echo Utilities::getLabel('M_Checkout')?> </a></div>		
           </div>  	
