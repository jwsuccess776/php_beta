     ...
     $(tabsId + ' A').click(function(){ 
        ...
        /*  if($(this).parent().data('filter') != 9999 )
		  ...
		  }else
		  {
		  	...
		  	var str='<p>Pay using Payfast: 4.5% will be added to total amount as payfast payment charges.</p><form method="post" action="/payfast_pay/order_charge/<?=$order_info["order_id"]?>" name="frmPaymentTabForm" id="frmPaymentTabForm" class="siteForm"><table width="100%"><tbody><tr><td colspan="2"><input type="submit" name="btn_submit" id="button-confirm" class="btn primary-btn" title="" style="margin: 10px 0;" value="CONFIRM PAYMENT"></td></tr></tbody></table><input type="hidden" name="order" value="<?=$order_info["order_id"]?>" title=""></form>';
