...
<?php }?>
    <style>
        ...
        .show_hide:hover .show_all_category{
            display: block;
        }
    </style>
</head>
  ...
  <div class="navpanel">
    <div class="fixed-container">
      ...
      <ul class="horizontal_links navigations">
          ...
          <?php
          //echo "<pre>";
          //print_r($header_categories1);
          //die();
          ?>
          <li class="show_hide"><a href="#" >All Category</a>
              <nav id="mysidebarmenu" class="amazonmenu" style="height: 0px;">
                  <ul class="show_all_category">
                      <?php foreach($header_categories[0] as $hkey=>$hval):?>
                      <li>
                          <a href="<?=Utilities::generateUrl('category','view',array($hval["category_id"]))?>"><?=Utilities::strip_javascript($hval["category_name"])?></a>
                          <ul>
                          <?php foreach($header_categories[$hval["category_id"]] as $subKey=>$subVal): $cntChild=0; $subChild++;
                              if($subChild<9):
                                  ?>
                              <li>
                                  <a href="<?=Utilities::generateUrl('category','view',array($subVal["category_id"]))?>"><?=$subVal["category_name"]?></a>
                                  <ul>
      	...
      	<?php
        foreach($header_categories[0] as $hkey=>$hval): $subChild=0; $primaryCategory++;?>
        <?php if ($primaryCategory<6): $sub_cats_count = count($header_categories[$hval["category_id"]]); ?>
        <li <?php if ($sub_cats_count>0) {?> class="navchild" <? } ?>>
            <a href="<?=Utilities::generateUrl('category','view',array($hval["category_id"]))?>"><?=Utilities::strip_javascript($hval["category_name"])?></a>
