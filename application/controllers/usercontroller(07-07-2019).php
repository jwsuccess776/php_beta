...
// ini_set('display_errors', 1);
        ...
        if ($fees_obj->num_rows > 0) {
            ...
            foreach ($data as $key => $value) {
                ...
                $user_id = $value['user_id'];
                $shop_id = $value['shop_id'];
                //echo $get_colm = "SELECT * FROM `tbl_url_column_setting` a WHERE user_id = '68' AND shop_id = '29' LIMIT 1";
                echo $get_colm = "SELECT * FROM `tbl_url_column_setting` a WHERE user_id = '$user_id' AND shop_id = '$shop_id' LIMIT 1";
                ...
                $colm_structure = array(
                      $get_colm_result_data[0]['prod_name'],                  
                      $get_colm_result_data[0]['prod_sku'],                  
                      $get_colm_result_data[0]['prod_slug'],                  
                      $get_colm_result_data[0]['prod_retail_price'],                  
                      $get_colm_result_data[0]['prod_sale_price'],                  
                      $get_colm_result_data[0]['prod_stock'],                  
                      $get_colm_result_data[0]['prod_long_desc'],                  
                      $get_colm_result_data[0]['prod_image'],                  
                );
                
                echo "<pre>";
                print_r($get_colm_result_data);
                echo "</pre>";     
                echo "=========";  
                die;
                $user_nodes = array();
                $rows = $this->xml_parser($value['url'],'rows',$colm_structure);
                echo $xml_node_count = $this->xml_node_count($value['url'],'rows',$colm_structure);
                //echo "<br>Total Rows";
               ...
               // $count_elemenet = count($rows[$value['prod_name']]);
                ...
                for ($i=0; $i < $count_elemenet; $i++) { 
                    /*echo "<pre>";
                print_r($rows[$value['prod_name']][$i]);
                echo "</pre>";*/
                ...
                $sku    = str_replace(',', '',$sku);
                   ...
                   $array[] = array(
                        ...
                        'prod_retail_price' =>(float)$rows[$value['prod_retail_price']][$i],
                        'prod_sale_price'   =>(float)$rows[$value['prod_sale_price']][$i],
                        'prod_stock'        =>$rows[$value['prod_stock']][$i],
                        'prod_long_desc'    =>$desc,
                        'prod_image'        =>$rows[$value['prod_image']][$i],
                    );
                }
         /*   echo "<pre>EXACT";
            ...
            foreach ($array as $ke => $val) {
                   ...
                   /* $select_query="select * from tbl_products where
                        prod_added_by = '".$this->getLoggedUserId()."' AND prod_sku ='".trim($val['prod_sku'])."' AND prod_name = '".trim($val['prod_name'])."' AND prod_shop='".$shop_id."'";
*/
                    $ordred_products=$this->db->query($select_query);
                    $ordred_product=$this->db->fetch_all($ordred_products);
                    $product_id = $ordred_product['0']['prod_id'];
    ...
    function xml_parser_1($url,$type,$avail_column=array()){
        set_time_limit(0);
        $xml = file_get_contents($url);
       // $data = simplexml_load_string($xml);
        ...
        if($data->channel)
        {
            ...
            die('proccess in xml parser');*/
             ...
             foreach($object->getElementsByTagName("item") as $item){ 
                $length = $item->childNodes->length;
                $total_avail_column = $avail_column;
                $total_avail_column_1 = $avail_column;
                /*echo"<pre>";
                echo $length;
                echo"</pre>";*/
                $i = 0;
    ...
    function xml_node_count_1($url,$type,$user_nodes=array()){
        set_time_limit(0);
        $xml = file_get_contents($url);
       // $data = simplexml_load_string($xml);
