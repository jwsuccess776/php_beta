...
function featured(){
	...
	$this->_template->render();	
	exit;
	//$criterias=array("status"=>1,"must_products"=>1,"pagesize"=>9,"page"=>1,"featured"=>1);
	$criterias=array("status"=>1,"must_products"=>1,"pagesize"=>9,"page"=>1);
	$frm = createHiddenFormFromPost("primarySearchForm",'',array(),$criterias);
	echo '<pre>';
	print_r($frm);
...
function view($shop_id,$category){
	...
	$page = isset($get['pg']) ? $get['pg'] : 1;	
	$searchKeyword = isset($get['shop_keyword']) ? $get['shop_keyword'] : "";
	$page = isset($get['shop_keyword']) ? 1 : $page;
	$criteria=array("shop"=>$shop_id,"category"=>$category,"status"=>1,"keyword="=>$searchKeyword,"pagesize"=>$pagesize,"page"=>$page,"donotspecial"=>1,"sort"=>$sort);
	...
	$pObj->setPageNumber($page);	
	$price_ranges = $pObj->getProductsMinMaxPrice(array_diff_key($arr_conditions, array_flip((array) array('min','max'))));
	$this->set('price_range', $price_ranges);

	$st = microtime(true);
	$categoryObj=new Categories();
	$shop_categories=$categoryObj->getProductCategoriesHavingRecordsOneLevel($category,$shop_id);
	$et = microtime(true);
	//echo 'Time Sub Category = ' . ($et - $st) . 's<br>';
    ...
    $et = microtime(true);
	//echo 'Time Card Payment Option = ' . ($et - $st) . 's<br>';
    $this->set('custom_payment_option', $custom_payment_option);
	
	$criteria=array("shop"=>$shop_id,"category"=>$category,"status"=>1,"keyword="=>$searchKeyword,"pagesize"=>$pagesize,"donotspecial"=>1,"sort"=>$sort);
	$arr_conditions = array_merge($criteria,$get);
$st = microtime(true);
		$pObj = new Products();
        ...
        $this->set('get', $get);	
    $et = microtime(true);
	//echo 'Time Product = ' . ($et - $st) . 's<br>';
    //die(0);
    
    //echo $primarySearchForm;
