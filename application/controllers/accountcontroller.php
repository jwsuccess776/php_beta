<?php
    ...
    class AccountController extends CommonController
    {
        ...
        function __construct($model, $controller, $action)
        {
			...
			//die($userObj->canAccessMyAccountPage($action)==false?"No":"Yes");
            ...
            $this->set('supplier_buyer_tab', $display_buyer_supplier_tab);
            //echo '<br/>panel : '.$display_buyer_supplier_tab;
//echo($display_buyer_supplier_tab."#".$_SESSION["buyer_supplier_tab"]);
            $_SESSION["buyer_supplier_tab"] = $display_buyer_supplier_tab;
            $this->set('rate_user_id', $user_id);
        ...
        function send_message_to_buyer($buyer_id, $order_id, $invoice_id)
        {
            $mydb = Syspage::getdb();
            $user_id = $this->getLoggedUserId();
            if (isset($_POST['btn_submit']))
            {
                ...
                /* $query="select * from tbl_order_products where opr_order_id ='$order_id' limit 1";
                  $rs=$mydb->query($query);
                  $thread_record=$mydb->fetch_all($rs); */

                $rating = $mydb->query("insert into tbl_threads (thread_subject,thread_started_by,thread_start_date,thread_type,thread_record) values ('" . $_POST['thread_subject'] . "', '" . $user_id . "', '" . date('Y-m-d') . "', 'O', '" . $order_id . "') ");
                $query = "select max(thread_id) as thread_id  from tbl_threads limit 1";
                $rs = $mydb->query($query);
                $max_id = $mydb->fetch_all($rs);

                //echo($max_id[0]['thread_id']);
                //exit;
                $rating = $mydb->query("insert into tbl_thread_messages (message_thread,message_from,message_to,message_text,message_date) values ('" . $max_id[0]['thread_id'] . "', '" . $user_id . "', '" . $buyer_id . "', '" . $_POST['message_text'] . "', '" . date('Y-m-d H:i:s') . "') ");

                $query = "select max(message_id) as message_id  from tbl_thread_messages limit 1";
                $rs = $mydb->query($query);
                $max_id = $mydb->fetch_all($rs);
                /* echo $max_id[0]['message_id'];
                  exit; */


                $emailNotificationObj = new Emailnotifications();
                $emailNotificationObj->SendMessageNotification($max_id[0]['message_id']);

                Message::addMessage('Your message has been sent successfully.');
                Utilities::redirectUser(Utilities::generateUrl('account', 'sales'));
            }
        ...
        function dashboard_buyer_x($_action = "", $_order_id = "", $referance_no = "")
        {

            if ($_action == 'view_payment_option' && $_order_id != "")
            {
                ...
                if ($referance_no == "")
                {
                    $orderObj = new Orders();
                    $userObj = new User();
                    $buyer_orders = $orderObj->getOrderById($_order_id);

                    // display only remaining balance if already paid
                    $buyer_orders['order_actual_paid'] = $buyer_orders['order_balance'];
                }
                else
                {
                    $db1 = Syspage::getdb();
                    $rs = $db1->query("select * from tbl_recharge_wallet_requests where rwr_id = '" . $_order_id . "' limit 1 ");
                    $_records = $db1->fetch_all($rs);

                    $buyer_orders['order_actual_paid'] = $_records[0]['rwr_amount'];
                    $buyer_orders['order_invoice_number'] = $_records[0]['rwr_invoice_number'];
                    $buyer_orders['order_reference'] = $_records[0]['rwr_referance_no'];
                }
                /* echo "<pre>";
                ...
                $this->set('_action', 'view_payment_option');
                $this->_template->render();
            }
            elseif ($_action == 'pay_now' && $_order_id != "")
            {
                ...
                //$order_detail["products"][] = $orderObj->getOrderProductsByOprId($_child_order);


                $this->set('_action', 'pay_now');
            ...
            else
            {
                ...
                /* echo '<pre>';
                  $arra=(array)Settings::getSetting("CONF_BUYER_ORDER_STATUS");
                  print_r($arra);
                  exit; */
                $orderObj = new Orders();
                $userObj = new User();
                ...
                /* $order_info1=$orderObj->getOrderById('75');
                  echo '<pre>';
                  print_r($order_info1);
                  echo '</pre>';
                  exit; */
        ...
        function dashboard_buyer($_action = "", $_order_id = "", $_child_order)
        {


            if ($_action == 'pay_now' && $_order_id != "")
            {
                ...
                $product = $orderObj->getOrderProductsByOprId($_child_order);
                $seller_shop_id = $product['opr_product_shop'];
                $this->set('shop_id', $seller_shop_id);
        ...
        function buyer_payment_options_x_1()
        {
            //ini_set('display_errors', 1);
//ini_set('display_startup_errors', 1);
//error_reporting(E_ALL);
            $mydb = Syspage::getdb();
            $buyer_id = $this->getLoggedUserId();
            $this->set('status', '0');
            //echo $buyer_id; 
            $rs = $mydb->query("SELECT * FROM tbl_buyer_payment_options where tbpo_buyer_id = '$buyer_id' limit 1");
            $row = $mydb->fetch_all($rs);

            if (count($row) > 0)
            {

                $this->set('status', $row[0]['tbpo_status']);
                if (isset($_GET['val']) && $_GET['val'] != "")
                {
                    $mydb->query("UPDATE tbl_buyer_payment_options SET tbpo_status='" . $_GET['val'] . "' WHERE tbpo_buyer_id='$buyer_id'");

                    Message::addMessage('Payment options status updated');
                    $dashboard_url = Utilities::generateUrl('account', 'buyer_payment_options');
                    Utilities::redirectUser($dashboard_url);
                }
            }
            else
            {
                if (isset($_GET['val']) && $_GET['val'] != "")
                {
                    /* echo "<pre>";
                      ...
                      echo $this->user_details['shop_id'];
                      exit; */

                    if (!empty($this->user_details['shop_id']))
                    {
                        $mydb->query("INSERT INTO tbl_buyer_payment_options (tbpo_buyer_id, tbpo_method_name, tbpo_status,tbpo_shop_id) VALUES ('$buyer_id', 'payfast', '" . $_GET['val'] . "', '" . $this->user_details['shop_id'] . "')");
                    ...
                    {
                        Message::addErrorMessage('Please create shop to continue.');
                        $dashboard_url = Utilities::generateUrl('account', 'shop');
                        Utilities::redirectUser($dashboard_url);
                    }
                }
            }
        ...
        function buyer_payment_options()
        {
            //ini_set('display_errors', 1);
//ini_set('display_startup_errors', 1);
//error_reporting(E_ALL);

            global $custom_payment_option;
            $mydb = Syspage::getdb();
            $buyer_id = $this->getLoggedUserId();


            if (isset($_GET['val']) && $_GET['val'] != '' && isset($_GET['pay_option']) && $_GET['pay_option'] != '')
            {
                $pay_option = $_GET['pay_option'];
                $val = $_GET['val'];
                $get_query = $mydb->query("SELECT * FROM tbl_buyer_payment_options where tbpo_buyer_id = '$buyer_id' AND tbpo_method_name='$pay_option' LIMIT 1");
                $get_result = $mydb->fetch_all($get_query);

                if (count($get_result) > 0)
                {
                    $tbpo_id = $get_result[0]['tbpo_id'];
                    $mydb->query("UPDATE tbl_buyer_payment_options SET tbpo_status='" . $val . "' WHERE tbpo_id='$tbpo_id'");
                    Message::addMessage('Payment options status updated');
                    $dashboard_url = Utilities::generateUrl('account', 'buyer_payment_options');
                    Utilities::redirectUser($dashboard_url);
                }
                ...
                {
                    $mydb->query("INSERT INTO tbl_buyer_payment_options (tbpo_buyer_id, tbpo_method_name, tbpo_status,tbpo_shop_id) VALUES ('$buyer_id', '$pay_option', '" . $val . "', '" . $this->user_details['shop_id'] . "')");
                    Message::addMessage('Payment options status updated');
                    $dashboard_url = Utilities::generateUrl('account', 'buyer_payment_options');
                    Utilities::redirectUser($dashboard_url);
                }
            ...
            foreach ($custom_payment_option as $key => $val)
            {
                $query = $mydb->query("SELECT * FROM tbl_buyer_payment_options where tbpo_buyer_id = '$buyer_id' AND tbpo_method_name='$key' LIMIT 1");
                $result = $mydb->fetch_all($query);

                if (count($result) > 0)
                {
                    $custom_payment_option[$key]['status'] = $result[0]['tbpo_status'];
                }
                else
                {
                    $custom_payment_option[$key]['status'] = '0';
                }
            }

            $this->set('custom_payment_option', $custom_payment_option);
            ...
            $this->set('status', '0');
            //echo $buyer_id; 
            $rs = $mydb->query("SELECT * FROM tbl_buyer_payment_options where tbpo_buyer_id = '$buyer_id'");
            $row = $mydb->fetch_all($rs);
        ...
        function buyer_rating()
        {

            $mydb = Syspage::getdb();

            if (isset($_POST['btnConfirmRate']))
            {
                $rating_mode = $_POST['rating_mode'];
                if ($rating_mode == 'add')
                {

                    $feedback = $_POST['feedback'];
                    $rating = $_POST['rating'];
                    $rate_order_product_id = $_POST['rate_order_product_id'];
                    $rate_order_id = $_POST['rate_order_id'];
                    $rate_seller_id = $_POST['rate_seller_id'];
                    $rate_buyer_id = $_POST['rate_buyer_id'];



                    $userObj = new User();
                    $buyer_details = $userObj->getUserById($rate_buyer_id, array("status" => 1));

                    $criteria = array("customer" => $rate_buyer_id, "status" => (array) Settings::getSetting("CONF_BUYER_ORDER_STATUS"), "pagesize" => 1);
                    $orderObj = new Orders();
                    $order_detail = $orderObj->getChildOrders($criteria);

                    $product_details = $mydb->query("select * from tbl_products where prod_id = '" . $order_detail['opr_product_id'] . "' limit 1");
                    $product_det = $mydb->fetch_all($product_details);


                    $rating = $mydb->query("insert into tbl_order_rating (tor_order_id,tor_seller_id,tor_buyer_id,tor_rating,tor_feedback, tor_opr_id) values ('" . $rate_order_id . "', '" . $rate_seller_id . "', '" . $rate_buyer_id . "', '" . $rating . "', '" . $feedback . "', '" . $rate_order_product_id . "' ) ");
                    Message::addMessage('Order rated successfully');


                    $arr_replacements = array(
                        '{site_domain}' => CONF_SERVER_PATH,
                        '{website_name}' => Settings::getSetting("CONF_WEBSITE_NAME"),
                        '{user_name}' => $buyer_details['user_username'],
                        '{username}' => $this->user_details['user_username'],
                        '{review_rating}' => $rating,
                        '{review_text}' => $feedback,
                        '{prod_title}' => $product_det[0]['prod_name'],
                    );


                    Utilities::sendMailTpl($buyer_details['user_email'], "product_review", $arr_replacements);
                }
                if ($rating_mode == 'update')
                {
                    $feedback = $_POST['feedback'];
                    $rating = $_POST['rating'];
                    $tor_id = $_POST['tor_id'];

                    $rating = $mydb->query("update tbl_order_rating set tor_rating = '" . $rating . "', tor_feedback = '" . $feedback . "' where tor_id = '" . $tor_id . "' ");
                    Message::addMessage('Order Rating Updated Successfully');
                }

                Utilities::redirectUser(Utilities::generateUrl('account', 'buyer_rating'));
            }
            ...
            if (isset($_GET['mode']) && ($_GET['mode']) == 'Search')
            {
                ...
                {
                    $subquery .= " and ( top.opr_name like '%" . $keyword . "%' or top.opr_brand like '%" . $keyword . "%' or  tor.order_invoice_number like '%" . $keyword . "%' or  tor.order_user_name like '%" . $keyword . "%'  ) ";
                }
                elseif ($keyword != "" && $date_from != "")
                {
                    $subquery .= " and ( ( top.opr_name like '%" . $keyword . "%' or top.opr_brand like '%" . $keyword . "%' or  tor.order_invoice_number like '%" . $keyword . "%' or  tor.order_user_name like '%" . $keyword . "%' ) and DATE(tor.order_date_added) >= '" . $date_from . "' ) ";
                }
                elseif ($keyword == "" && $date_from != "" && $date_to == "")
                {
                    $subquery .= " and ( DATE(tor.order_date_added) >= '" . $date_from . "' ) ";
                }
                elseif ($keyword == "" && $date_from != "" && $date_to != "")
                {
                    $subquery .= " and ( DATE(tor.order_date_added) between '" . $date_from . "' and  '" . $date_to . "' ) ";
                }

                /* if($status!="")
                  {
                  $subquery.= " and ( tor.order_status = '".$status."' )";
                  } */
            }

            $query = "select * from tbl_shops ts, tbl_order_products top, tbl_users tcu, tbl_orders tor where ts.shop_user_id = '" . $user_id . "' and top.opr_product_shop = ts.shop_id and top.opr_order_id = tor.order_id and tor.order_user_id  != '" . $user_id . "' and tor.order_user_id = tcu.user_id  " . $subquery . ' order by tor.order_date_added desc';
            ...
            $orderObj = new Orders();
            $sales_order = $orderObj->getChildOrders($criterias);
            $this->set('order_rows', $order_rows);
            $this->set('arr_listing', $sales_order);
            $this->set('pages', $orderObj->getTotalPages());
            $this->set('page', $page);
            $this->set('start_record', ($page - 1) * $pagesize + 1);
        ...
        function buyer_rating_details()
        {
            $mydb = Syspage::getdb();
            $tor_id = $_REQUEST['tor_id'];
            $response = array(
                'status' => 'error'
            );
            $rs = $mydb->query("select * from tbl_order_rating where tor_id= '" . $tor_id . "' limit 1");
            $row = $mydb->fetch_all($rs);
            if (count($row[0]) > 0)
            {
                ...
                $response = array(
                    'status' => 'success',
                    'tor_id' => $row[0]['tor_id'],
                    'tor_order_id' => $row[0]['tor_order_id'],
                    'tor_seller_id' => $row[0]['tor_seller_id'],
                    'tor_buyer_id' => $row[0]['tor_buyer_id'],
                    'tor_rating' => $row[0]['tor_rating'],
                    'tor_feedback' => $row[0]['tor_feedback'],
                );
            }
        ...
        function sales_view_order($opr_id)
        {
            ...
            {
                $mapping = ($osObj->getOrderStatuses(array('digital' => 0)));
                $order_statuses = $this->get_values_for_keys($mapping[0], "orders_status_id");
        ...
        function seller_cancellation_request($id)
        {
            $userObj = new User();
            $user_id = $this->getLoggedUserId();
            $orderObj = new Orders();
            $order_detail = $orderObj->getOrderProductsByOprId($id, array("vendor" => $user_id, "status" => (array) Settings::getSetting("CONF_VENDOR_ORDER_STATUS")));
            //$order_detail = $orderObj->getChildOrders($criteria);
            ...
            if (in_array($order_detail["opr_status"], $arr_pro_com))
            {
                ...
                $order_eligible_cancel_request = true;
                $this->set('not_eligible', $order_eligible_cancel_request);
            }			
            ...
            $this->set('order_detail', $order_detail);
            $frm = $this->getSellerCancelRequestForm($order_detail);
            $post = Syspage::getPostedVar();
            /* echo '<pre>';
              print_r($frm);
              echo $order_eligible_cancel_request;
              print_r($criteria);
              echo '</pre>';
              exit; */
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($post['btn_submit']) && ($order_eligible_cancel_request))
            {
                ...
                else
                {
                    ...
                    if ($userObj->addOrderCancellationRequestSupplier($arr))
                    {
                        $emailNotificationObj = new Emailnotifications();
                        if ($emailNotificationObj->SendSellerOrderCancellationNotification($userObj->getCancellationRequestId()))
                        {
        ...
        function cancel_order($opr_id)
        {
            ...
            $order_detail["comments"] = $orderObj->getOrderComments(array("opr" => $opr_id));
            $arr_pro_com = $orderObj->getNotAllowedOrderCancellationStatuses();
			//$arr_pro_com = $orderObj->getBuyerAllowedOrderCancellationStatuses();
            if (in_array($order_detail["opr_status"], $arr_pro_com))
            {
                Message::addErrorMessage(sprintf(Utilities::getLabel('L_this_order_already'), "<i>" . $order_detail["orders_status_name"] . "</i>"));
                $not_eligible = true;
        ...
        protected function getRechargeWalletForm()
        {
            ...
            $frm->captionInSameCell(true);
            $frm->setRequiredStarWith('not-required');
            $fldMinPrice = $frm->addRequiredField('', 'amount', '', '', ' id="walletAmt" class="enter-amount" placeholder="' . Utilities::getLabel('L_Enter_Amount_to_be_Added') . '"');
            ...
            $fldButton = $frm->addButton('', 'btn_submit1', Utilities::getLabel('L_Add_Money_to_Wallet'), '', 'class="btn primary-btn" id="btnConfirmPopup"');
            $fldButton = $frm->addSubmitButton('', 'btn_submit', Utilities::getLabel('L_Add_Money_to_Wallet'), '', 'class="btn primary-btn" style="display:none" id="btn_Add_Money_to_Wallet"');
        ...
        function credits()
        {
            ...
            $query = "select * from tbl_recharge_wallet_requests where rwr_member_id = '" . $user_id . "' order by  FIELD(rwr_payment_status, 'Pending') desc, rwr_date desc";
        ...
        private function getWithdrawalForm()
        {
            ...
            $captiotn = array(
                ...
                1 => Array(
                    'group_caption' => 'All(may incur additional fees)',
                    'options' => array(
                        ...
                        'CitiBank' => 'CitiBank',
                        'FinBond Mutual Bank' => 'FinBond Mutual Bank',
                        ...
                        'Lesotho Post Bank' => 'Lesotho Post Bank',
                        'Mercantile Bank' => 'Mercantile Bank',
                        'Olympus Mobile' => 'Olympus Mobile',
                        'Peoples Bank Ltd Inc NBS' => 'Peoples Bank Ltd Inc NBS',
                        'PostBank' => 'PostBank',
                        'S.A. Reserve Bank' => 'S.A. Reserve Bank',
                        'SASFIN Bank' => 'SASFIN Bank',
                        'Standard Chartered Bank' => 'Standard Chartered Bank',
                        'State Bank of India' => 'State Bank of India',
                        'UBank Limited' => 'UBank Limited',
                        'Unibank' => 'Unibank',
                        'VBS Mutual Bank' => 'VBS Mutual Bank',
                    )
                )
            );
            $frm->addSelectBox('<label>' . Utilities::getLabel('M_Bank_Name') . '</label>', 'ub_bank_name', $captiotn, '', 'required', Utilities::getLabel('M_Bank_Name'), 'ub_bank_name')->requirements()->setRequired();
            ...
            $frm->addRequiredField('<label>Branch Code</label>', 'ub_ifsc_swift_code', null, 'ub_ifsc_swift_code');
            ...
            $captiotn = array(
                '0' => Array(
                    'options' => array(
                        ...
                        'Transmission account' => 'Transmission account',
            ...
            $frm->addSelectBox('<label>Bank Account Type</label>', 'ub_bank_account_type', $captiotn, '', 'required', 'Bank Account Type', 'ub_bank_account_type')->requirements()->setRequired();
        ...
        function orders()
        {
            ...
            if ($post['btn_rcnt'])
            {
                $wks_ago = date("Y-m-d", strtotime('-2 weeks'));
                // print_r($onewk_ago);exit;
                $post = Array('keyword' => '', 'status' => '', 'date_from' => $wks_ago, 'date_to' => $crnt_day, 'minprice' => '', 'maxprice' => '', 'btn_submit' => 'Search', 'mode' => 'search', 'page' => '1');
            }
            if ($post['btn_mnt'])
            {
                $mnt_ago = date("Y-m-d", strtotime('-1 month'));
                // print_r($wks_ago);exit;
                $post = Array('keyword' => '', 'status' => '', 'date_from' => $wks_ago, 'date_to' => $crnt_day, 'minprice' => '', 'maxprice' => '', 'btn_submit' => 'Search', 'mode' => 'search', 'page' => '1');
            }
            if ($post['btn_3mnts'])
            {
                $mnts_ago = date("Y-m-d", strtotime('-3 month'));
                // print_r($mnts_ago);exit;
                $post = Array('keyword' => '', 'status' => '', 'date_from' => $mnts_ago, 'date_to' => $crnt_day, 'minprice' => '', 'maxprice' => '', 'btn_submit' => 'Search', 'mode' => 'search', 'page' => '1');
            }
            if ($post['btn_year'])
            {
                $year_ago = date("Y-m-d", strtotime('-1 year'));
                // print_r($year_ago);exit;
                $post = Array('keyword' => '', 'status' => '', 'date_from' => $year_ago, 'date_to' => $crnt_day, 'minprice' => '', 'maxprice' => '', 'btn_submit' => 'Search', 'mode' => 'search', 'page' => '1');
            }
            ...
            //Customization
            $getrecentform = $this->getrecentform();
            //
            $frmSearchForm->fill($arr);
            if ($page < 1)
                $page = 1;
            ...
            if ($post['btn_rate'])
            {
                $orderLen = count($sales_order);
                $productFeedbackObj = new Productfeedbacks();
                for ($i = 1; $i <= $orderLen; $i++)
                {
                    $flag = 0;
                    $product_feedback = $productFeedbackObj->getProductFeedback('', array("order" => $sales_order[$i]["opr_id"]));
                    if (($product_feedback == true))
                    {
                        unset($sales_order[$i]);
                        $flag++;
                    }
                    if (!in_array($sales_order[$i]["opr_status"], (array) Settings::getSetting("CONF_REVIEW_READY_ORDER_STATUS")))
                    {
                        unset($sales_order[$i]);
                        $flag++;
                    }
        ...
        protected function getBuyerSearchForm()
        {
            ...
            $frm->addSubmitButton('', 'btn_rate', 'Rating Required', '', 'class="btnorder primary-btn"');
        ...
        //Customization
        protected function getrecentform()
        {
            $frmnew = new Form('getrecentform', 'getrecentform');
            $frmnew->setFieldsPerRow(5);
            $frmnew->setExtra('class="siteFormorder ondark" rel="search"');
            $frmnew->captionInSameCell(true);
            $frmnew->setRequiredStarWith('not-required');
            $frmnew->addHiddenField('', 'mode', "search");
            $frmnew->addHiddenField('', 'page', '1');
            //Customization
            //$frmnew->addSubmitButton('','btn_rcnt','recent orders','','class="btnorder primary-btn"');
            //$frmnew->addSubmitButton('','btn_mnt','orders in last month','','class="btnorder primary-btn"');
            //$frmnew->addSubmitButton('','btn_3mnts','orders in last 3 months','','class="btnorder primary-btn"');
            //$frmnew->addSubmitButton('','btn_year','orders in last year','','class="btnorder primary-btn"');
            //$frmnew->addSubmitButton('','btn_rate','Orders require rating','','class="btnorder primary-btn"');
            //
		$frmnew->setJsErrorDisplay('afterfield');
            $frmnew->setAction('?');
            $this->set('frmnew', $frmnew);
            return $frmnew;
        }
        ...
        function feedback($id)
        {
            ...
            // print_r((array)Settings::getSetting("CONF_REVIEW_READY_ORDER_STATUS"));exit;
            $orderObj = new Orders();
            $order_detail = $orderObj->getChildOrders($criteria);
        ...
        function return_request($id)
        {
            ...
            //print_r((array)$orderObj->getBuyerAllowedOrderReturnStatuses());
            //exit;
        ...
        function ajax_product_list()
        {
            // if ($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                ...
                $criteria = array("shop" => $user_shop_id, 'added_by' => $user_id, "sort" => "rece");
                ...
                // $psObj->joinWithTotalViewTable(); // Added 23-08-2018
                ...
                $arr_listing = $psObj->getProducts($criteria);
                $pages = $psObj->getTotalPages();                
                ...
                $page_info = sprintf(Utilities::getLabel('L_Items_x_to_y_of_z_total'),$start_record,$end_record,$total_records) . '&&';
                echo $page_info;
...
?>                
                <tr>
                    <td><div class="pro-image"><img onerror="this.src='<?php echo Utilities::generateUrl('image','product_image',array($row["prod_id"],'THUMB'))?>'" src="<?php echo $row['prod_image']?>" alt="<?php echo $row["prod_name"]?>"/> </a></div></td>
                    ...
                    <td>
                        <span class="cellcaption"><?php echo Utilities::getLabel('L_Sold')?></span> <?php echo $row["prod_sold_count"] ?></td>
...
<?php           
        ...
        {         
            ...
            $shopObj = new Shops();
            $shop = $shopObj->getData($user_shop_id, array("is_owner" => 1));
         
            if ($shop['shop_status'] != 1)
            {
                Message::addErrorMessage(Utilities::getLabel('M_ADMIN_HAS_DIABLED_YOUR_SHOP'));
            }
        ...
        function shippingclass_publications()
        {
            ini_set('display_errors', 1);
            $post = Syspage::getPostedVar();
            $page = intval($post["page"]);
            ...
            $productObj = new Products(false, true);
            $check_assion_product = array();
            foreach ($psObj->getShippingclass($criteria) as $sn => $row)
            {
                $check_assion_product[$row['id']] = $productObj->check_assion_class($row['id']);
            }
            $this->set('check_assion_product', $check_assion_product);
        ...
        function view_return_request($id)
        {
            ...
            /* Utilities::printarray($request_detail);
              die(); */
            $this->set('frm', $frm);
            $this->set('request_detail', $request_detail);
            $this->_template->render();
        ...
        function remove_shipping_class($id)
        {
            $this->pauseShippingClassProduct($id);

            $user_id = $this->getLoggedUserId();
            $prodObj = new Shippingclass(false, true);
            $product_details = $prodObj->getShippingclass(array("shippingclass_added_by" => $user_id, "id" => $id));

            if ($product_details == true)
            {
                if ($prodObj->deleteShippingClass(intval($id), $user_id))
                {
                    Message::addMessage(Utilities::getLabel('M_Record_deleted'));
                }
                else
        ...
        function pauseShippingClassProduct($id)
        {
            $mydb = Syspage::getdb();
            $prod_query = "select * from tbl_products where shipping_class_id = " . $id . " ";
            $prod_dets = $mydb->query($prod_query);
            //print_r($commission_dets);
            if ($prod_dets->num_rows > 0)
            {
                $prod_res = $mydb->fetch_all($prod_dets);
                foreach ($prod_res as $com_row)
                {
                    $product_id = $com_row['prod_id'];
                    $mod = 'block';
                    
                    $prodObj = new Products(false, true);
                    $user_id = $this->getLoggedUserId();
                    $product_details = $prodObj->getProducts(array("added_by" => $user_id, "id" => $product_id));

                    if ($product_details == true)
                    {
                        if ($prodObj->updateProductStatus(intval($product_id), $mod))
                        {
        ...
        function address_form($ua_id = 0, $ajax_request = 0)
        {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($post['btn_submit']))
            {
                ...
                else
                {
                    ...
                    if ($addressID = $userObj->addUpdateAddress($arr))
                    {
                        if (!$ajax_request)
                        {
        ...
        protected function getAddressForm()
        {
            ...
            $frm->addSelectBox('<label>' . Utilities::getLabel('M_State_County_Province') . '</label>', 'ua_state', $stateObj->getStatesAssoc(Settings::getSetting("CONF_COUNTRY")), '', 'onchange="loadCities(this);"', Utilities::getLabel('M_State_County_Province'), 'ua_state')->requirements()->setRequired(true);
            $frm->addSelectBox('<label><strong>' . Utilities::getLabel('M_City_Town') . '</strong></label>', 'ua_city', '', '', '', Utilities::getLabel('M__City_Town'), 'ua_city')->requirements()->setRequired(true);
        ...
        function address_form_before_city($ua_id = 0, $ajax_request = 0)
        {
            $userObj = new User();
            $ua_id = intval($ua_id);
            $frm = $this->getAddressForm();
        ...
        protected function getAddressForm_before_city()
        {
            global $loggedin_user;
            $frm = new Form('frmAddress');
            $frm->addHiddenField('', 'ua_id', 0);
        ...
        function profile_info_before_ci()
        {
            Syspage::addJs(array('js/jquery.form.js'), false);
            Syspage::addJs(array('js/cropper.js'), false);
            Syspage::addCss(array('css/cropper.css'), false);
        ...
        private function getProfileInfoForm_before_city()
        {
        ...
        private function getProfileInfoForm()
        {
            ...
            $stateObj = new States();
            $fld_country = $frm->addSelectBox('<label><strong>' . Utilities::getLabel('M_Country') . '</strong></label>', 'user_country', $countryObj->getAssociativeArray(), Settings::getSetting("CONF_COUNTRY"), 'onchange="loadStates(this);"', Utilities::getLabel('M_Country'), 'ua_country')->requirements()->setRequired(true);
            $frm->addSelectBox('<label><strong>' . Utilities::getLabel('M_State_County_Province') . '</strong></label>', 'ua_state', '', '', 'onchange="loadCities(this);"', Utilities::getLabel('M_State_County_Province'), 'ua_state')->requirements()->setRequired(true);
            $frm->addSelectBox('<label><strong>' . Utilities::getLabel('M_City_Town') . '</strong></label>', 'ua_city', '', '', '', Utilities::getLabel('M__City_Town'), 'ua_city')->requirements()->setRequired(true);
          //  $frm->addRequiredField('<label><strong>' . Utilities::getLabel('M_City_Town') . '</strong></label>', 'user_city_town', null, 'user_city_town');
            $frm->addHiddenField('', 'action', 'avatar', 'avatar-action');
            $frm->addHiddenField('', 'img_data', '', 'img-data');
        ...
        private function getBankInfoForm()
        {
            ...
            $frm->addRequiredField('<label>' . Utilities::getLabel('M_Account_Holder_Name') . '</label>', 'ub_account_holder_name', null, 'user_account_holder_name');
            $frm->addRequiredField('<label>' . Utilities::getLabel('M_Account_Number') . '</label>', 'ub_account_number', null, 'user_account_number');
            $frm->addTextBox('<label>Branch Code</label>', 'ub_ifsc_swift_code', null, 'ub_ifsc_swift_code');
        ...
        private function getShopInfoForm()
        {
            ...
            $frm->addTextArea('<label>' . Utilities::getLabel('L_Payment_Policy') . '</label>', 'shop_payment_policy', '', 'shop_payment_policy', ' class="height120" rows="3" maxlength="2000" ');
            $frm->getField("shop_payment_policy")->html_after_field = Utilities::getLabel('L_Payment_Policy_Text');
            $frm->addTextArea('<label>' . Utilities::getLabel('M_Delivery_Policy') . '</label>', 'shop_delivery_policy', '', 'shop_delivery_policy', ' class="height120" rows="3"  maxlength="2000" ');
            $frm->getField("shop_delivery_policy")->html_after_field = '<small>' . Utilities::getLabel('M_Delivery_Policy_Text') . '</small>';
            $frm->addTextArea('<label>' . Utilities::getLabel('L_Refund_Policy') . '</label>', 'shop_refund_policy', '', 'shop_refund_policy', ' class="height120" rows="3" maxlength="2000" ');
            $frm->getField("shop_refund_policy")->html_after_field = Utilities::getLabel('L_Refund_Policy_Text');
            $frm->addTextArea('<label>' . Utilities::getLabel('M_Additional_Information') . '</label>', 'shop_additional_info', '', 'shop_additional_info', ' class="height120" rows="3" maxlength="2000" ');
            $frm->getField("shop_additional_info")->html_after_field = '<small>' . Utilities::getLabel('M_Additional_Information_Text') . '</small>';
            $frm->addTextArea('<label>' . Utilities::getLabel('L_SELLER_INFORMATION') . '</label>', 'shop_seller_info', '', 'shop_seller_info', ' class="height120" rows="3" maxlength="2000" ');
        ...
        function shipping_class($shippingclass_id = 0, $tab = "")
        {
            ini_set('display_errors', 1);
            ...
            $user_id = $this->getLoggedUserId();
            $userObj = new User();
            $shippingclassObj = new Shippingclass(false, true);
            ...
            $shippingclass_id = intval($shippingclass_id);

            $shippingclass = array('shippingclass_added_by' => $this->getLoggedUserId(), 'shippingclass_shop' => $user_details["shop_id"]);

            /* $shippingclass = $shippingclassObj->getData($shippingclass_id,array("shippingclass_added_by"=>$this->getLoggedUserId()));
              print_r($shippingclass);
              die; */

            if ($shippingclass_id > 0 && $shippingclass = $shippingclassObj->getData($shippingclass_id, array("shippingclass_added_by" => $this->getLoggedUserId())))
            {

                $shippingclass['class_name'] = $shippingclass['class_name'];

                /* $countryObj=new Countries();
                  $country_info = $countryObj->getData($shippingclass["prod_shipping_country"]);
                  $shippingclass["shipping_country"]=$country_info["country_name"]; */

                $shippingclass["shop"] = $shippingclass["shop_name"];

                /* $prodObj=new Products();
                  $filters = $prodObj->getProductFilters($shippingclass_id);
                  $shippingclass['product_filters'] = array();
                ...
                $shipping_rates = $shippingclassObj->getShippingClassShippingRates($shippingclass_id);
                $shippingclass['product_shipping_rates'] = array();
                foreach ($shipping_rates as $key => $val)
                {
                    $shippingclass['product_shipping_rates'][] = $val;
                }

                $this->set('data', $shippingclass);
            }
            else
            {
                if (!$shippingclass)
                {
                    ...
                    Utilities::redirectUser(Utilities::generateUrl('account', 'publications'));
                }
            }

            $shippingclass['shippingclass_id'] = $shippingclass_id;

            $frm = $this->getShippingclassForm($shippingclass);
            //unset($product['prod_category']);
            $frm->fill($shippingclass);
            $post = Syspage::getPostedVar();
            if ($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                //p($_POST);
                ...
                {
                    $is_everything_else = false;

                    foreach ($post['product_shipping'] as $fields)
                    {
                        if ($fields['state_id'] == -1)
                        {
                            $is_everything_else = true;
                            $error = false;
                        }
                    }
                    if ($is_everything_else != true)
                    {
                        Message::addErrorMessage(Utilities::getLabel('M_ERROR_INVALID_REQUEST'));
                        $error = Utilities::getLabel('M_ERROR_INVALID_REQUEST');
                    }
                    ...
                    if (!$error)
                    {
                        $post = array_merge(array('shippingclass_added_by' => $this->getLoggedUserId(), 'shippingclass_shop' => $user_details["shop_id"]), $post);
                        if ($shippingclassObj->addUpdateShippingClass($post))
                        {
                            $shippingclass_id = $shippingclassObj->getShippingClassId();
                            Message::addMessage(Utilities::getLabel('M_YOUR_ACTION_PERFORMED_SUCCESSFULLY'));
                            Utilities::redirectUser(Utilities::generateUrl('account', 'shipping_class', array($shippingclass_id)));
                        }
                        else
                        {
                            Message::addErrorMessage(Utilities::getLabel('M_ERROR_INVALID_REQUEST'));
            ...
            $this->set('shippingclass_id', $shippingclass_id);
        ...
        function shipping_class_before_ci($shippingclass_id = 0, $tab = "")
        {
            ini_set('display_errors', 1);
        ...
        function product_form($prod_id = 0, $tab = "")
        {
            ...
            $userObj = new User();
            $prodObj = new Products(false, true);
            //$prodObj->joinWithShippingClassTable();
            $prodObj->joinWithDetailTable();
            $prodObj->joinWithURLAliasTable();
            $prodObj->joinWithBrandsTable();
            $prodObj->joinWithCategoryTable();
            ...
            if ($prod_id > 0 && $product = $prodObj->getData($prod_id, array("added_by" => $this->getLoggedUserId())))
            {
                ...
                $shippingclassObj = new Shippingclass();
                $shippingclassObj_info = $shippingclassObj->getData($product["shipping_class_id"]);
                $product["class_name"] = $shippingclassObj_info["id"];
        ...
        function product_form_before_city($prod_id = 0, $tab = "")
        {
            // ini_set('display_errors', 1);
            if ($prod_id == 0 && !$this->canAccessProductsArea)
            {
        ...
        private function getShippingclassForm($info)
        {

            global $binary_status, $active_inactive_status, $ship_to_states;
            global $conf_length_class, $conf_weight_class, $binary_status;

            $frm = new Form('frmShippingclass', 'frmShippingclass');
            $frm->setOnSubmit('return validateShippingclassForm(this, ShippingclassfrmValidator);');
            $frm->setExtra(' validator="ShippingclassfrmValidator" class="siteForm" autocomplete="off"');
            $frm->setValidatorJsObjectName('ShippingclassfrmValidator');
            $frm->setJsErrorDisplay('afterfield');
            $frm->setTableProperties('class="tbl-twocell"');
            $frm->setLeftColumnProperties('width="20%"');
            $frm->addHiddenField('', 'shippingclass_id', 0, 'shippingclass_id');
            $frm->addHiddenField('', 'shippingclass_added_by');
            $frm->addHiddenField('', 'shippingclass_shop');

            /*             * ***************    START TAB 1      ****************** */
            $fld = $frm->addRequiredField(Utilities::getLabel('M_CLASS_NAME'), 'class_name', '', 'class_name', ' class="medium"');

            // $fld->requirements()->setIntPositive($val=true);
            /* $fld->requirements()->setIntPositive();
              $fld->requirements()->setRegularExpressionToValidate('^[1-9][0-9]$'); */

            /* $frm->addSelectBox(Utilities::getLabel('M_SHIP_TO_STATE'), 'ship_to_state_id',$ship_to_states,'1','class="medium"','','ship_to_state_id');
        ...
        private function getProductForm($info)
        {
            ...
            $cObj = new Categories();
            $cat_arr = array_reverse($cObj->funcGetCategoryStructure($info["prod_category"]));
            ...
            //$fld=$frm->addTextBox(Utilities::getLabel('M_Dimensions_LWH'), 'prod_length','', '', 'class="medium"');

            $fldLength = $frm->addTextBox(Utilities::getLabel('M_Dimensions_LWH'), 'prod_length', '', '', 'Placeholder="' . Utilities::getLabel('M_Length') . '" ');
            //$fldLength->html_after_field = ' ';
            $fldWidth = $frm->addTextBox(Utilities::getLabel('M_Dimensions_LWH'), 'prod_width', '', '', 'Placeholder="' . Utilities::getLabel('M_Width') . '" ');
            //	$fldWidth->html_after_field = ' ';
            $fldHeight = $frm->addTextBox(Utilities::getLabel('M_Dimensions_LWH'), 'prod_height', '', '', 'Placeholder="' . Utilities::getLabel('M_Height') . '" ');
            ...
            $field_weight = $frm->addTextBox(Utilities::getLabel('M_Weight'), 'prod_weight', '', '', 'Placeholder="' . Utilities::getLabel('M_Weight') . '" ');
            ...
            $shippingClassObj = new Shippingclass();
            $user_id = $this->getLoggedUserId();
            $shipping_classes = $shippingClassObj->getAssociativeArrayBySeller($user_id);

            $frm->addSelectBox(Utilities::getLabel('M_CLASS_NAME'), 'class_name', $shipping_classes, 'class="medium"', '', 'NONE', 'class_name');
        ...
        private function getSellerCancelRequestForm($info)
        {
            $frm = new Form('frmCancelRequestForm', 'frmCancelRequestForm');
            $frm->setExtra('class="siteForm"');
            $frm->setLeftColumnProperties('width="30%"');
            ...
            //$fld->requirements()->setRequired(true);
            //$fld->requirements()->setCustomErrorMessage(Utilities::getLabel('L_Please_select_your_reason'));
            $fld = $frm->addTextArea('<label>' . Utilities::getLabel('L_Reason_for_cancellation') . '</label>', 'message');
            $fld->requirements()->setRequired(true);
            $fld_submit = $frm->addSubmitButton(null, 'btn_submit', Utilities::getLabel('M_Send_Request'), 'btn_submit', '');
            return $frm;
        }		
        ...
        function send_email()
        {
            ...
            if (count($email) && !empty($email))
            {
                ...
                foreach ($email as $email_id)
                {
                    ...
                    if (!Utilities::isValidEmail($email_id))
                        continue;
...
$frm->setOnSubmit('return setUpBrandRequest(this, RequestfrmValidator);');
...
function shipping_class_setup_info(){
$extraPageObj=new Extrapage();
$arr_shipping_class_setup_content=$extraPageObj->getExtraBlockData(array('identifier'=>'SHIPPING_CLASS_SETUP_CONTENT_BLOCK'));
$shipping_class_setup_content=$arr_shipping_class_setup_content["epage_content"];

$this->set('shipping_class_setup_content', $shipping_class_setup_content);
$this->_template->render(false,false);
}
...
$frm->setOnSubmit('return setUpTagRequest(this, TagRequestfrmValidator);');
$frm->setValidatorJsObjectName('TagRequestfrmValidator');
$frm->setExtra('class="siteForm"');
$frm->setLeftColumnProperties('width="30%"');
$frm->setJsErrorDisplay('afterfield');
$frm->setRequiredStarWith('caption');
...
die(Utilities::getLabel('L_Image_with_orientation_saved'));
        ...
        function shoplayouts(){
            ...
            if ($shop = $shopObj->getShopsByCriteria(array("user" => $this->getLoggedUserId(), "is_owner" => 1), 1)) {
                ...
                if(isset($_POST['shop_template']) && !empty($_POST['shop_template'])){
                    $shopTemplate = $_POST['shop_template'];
                    $shopID = $shop['shop_id'];

                    if($shopTemplate && $shopID){
                        $mydb->query("UPDATE tbl_shops SET shop_template='" . $shopTemplate . "' WHERE shop_id='$shopID'");
                        $shop['shop_template'] = $shopTemplate;
                    }
                }

                $this->set('shop_template', $shop['shop_template']);
                $this->_template->render();
            }else {
                $this->set('shop_template', 1);
                $this->_template->render();
            }
        }
