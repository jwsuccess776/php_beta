...
//$this->set('checked_conditions',array("N"));
        ...
        protected function product_count($prod_id) {

            // gets the user IP Address
            $user_ip = $_SERVER['REMOTE_ADDR'];
            $mydb = Syspage::getdb();
            $query_product_view = $mydb->query("select count(user_ip) as total_entry from tbl_product_view where prod_id='$prod_id' and user_ip='$user_ip'");
            $check_ip = $mydb->fetch_all($query_product_view);
            $total_entry = $check_ip[0]['total_entry'];
            $conf_particular_ip = Settings::getSetting("CONF_ENABLE_VIEW_PARTICULAR_IP");
    
            if (($total_entry >= '1') && (!empty($conf_particular_ip))) {
                
            } else {
    
                $date = date('Y-m-d H:i:s');
                $insertview = $mydb->query("insert into tbl_product_view values('','$prod_id','$user_ip','$date')");
                $query_product_count = $mydb->query("select count(prod_id) as product_entry from tbl_product_total_view where prod_id='$prod_id'");
                $check_count_entry = $mydb->fetch_all($query_product_count);
                $product_entry = $check_count_entry[0]['product_entry'];
                if ($product_entry >= '1') {
                    $updateviewcount = $mydb->query("update tbl_product_total_view set total_visit = total_visit+1 where prod_id='$prod_id' ");
                } else {
                    $insertviewcount = $mydb->query("insert into tbl_product_total_view set prod_id='$prod_id',total_visit ='1'");
                }
            }
        }
        ...
        function view($prod_id)
        {
            ...
            if (empty($shipping_rates) && $product['shipping_class_id'] != 0)
            {
                $shipping_rates = $pObj->getShippingClassById($prod_id, array("shipping_class_id" => $product['shipping_class_id']));
            }
            //p($shipping_rates);
            ...
            //Added on 20 august for payment options
            global $custom_payment_option;
            $mydb = Syspage::getdb();
            $shop_user_id = $product['shop_user_id'];
            foreach ($custom_payment_option as $key => $val) {
                $query = $mydb->query("SELECT * FROM tbl_buyer_payment_options where tbpo_buyer_id = '$shop_user_id' AND tbpo_method_name='$key' LIMIT 1");
        ...
        {   
            ...
            $pObj->selectFields(array('tp.*','ts.*','tu.*','IF(prod_stock >0, "1", "0" ) as available'));
            $products = $pObj->getProducts($arr_conditions);
           /* echo "<pre>";
            print_r($products);
            die;*/
