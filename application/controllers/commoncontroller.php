...
//'order'=>array('detail'),
	...
	protected function setTemplateValues($tpl){
		...
		if($common_elements == null) {
			...
			$header_categories1=$cat->getParentAssociativeArray(0,1);
			...
			$common_elements=array(
									...
									"header_categories1"=>$header_categories1,
...
function ajax_show_products_json(){
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$pagesize = isset($post['pagesize'])?$post['pagesize']:10;
		$post['pagesize'] = $pagesize;
		if(isset($post['shop_keyword']))
		{
			$post['keyword'] = $post['shop_keyword'];
		}
		...
		if ($post['page']==1){
			...
			if ($post['brand_page']!=1){
				...
				$brands = $pObj->getProductsSearchedBrands(array('category'=>$category_id,"min"=>$post['min'],"max"=>$post['max'],"keyword"=>$post['keyword'],"property"=>array(),"tags"=>$post['tags']),array_diff_key($conditions,array_flip(array("brand"))));	
...
if($post['sort']=='best')
{
	foreach ($arr_products as $key => $prod_row) {
    	$sort_arr[$key] = $prod_row['prod_sold_count'];
	}
	array_multisort($sort_arr, SORT_DESC , $arr_products);
}elseif($post['sort']=='plth')
{
	foreach ($arr_products as $key => $prod_row) {
    	$sort_arr[$key] = $prod_row['prod_sale_price'];
	}
	array_multisort($sort_arr, SORT_ASC , $arr_products);
}elseif($post['sort']=='phtl')
{
	foreach ($arr_products as $key => $prod_row) {
    	$sort_arr[$key] = $prod_row['prod_sale_price'];
	}
...
elseif($post['sort']=='rece')
{
	foreach ($arr_products as $key => $prod_row) {
    	$sort_arr[$key] = $prod_row['prod_id'];
	}
	array_multisort($sort_arr, SORT_DESC , $arr_products);
}			
...
public function loadDropDown_before_city($parent_id = 0, $type = ''){
	$states = new States();
	$dd = array();
...
public function loadDropDown($parent_id = 0, $type = ''){
	...
	switch(strtoupper($type)){
		...
		$dd = $subPack->getAssocSubPackages($criteria);
                case 'CITIES':
		$dd = $cities->getCitiesAssoc($parent_id);
		break;                    
		default:
		$dd = array();
	}
...
//echo json_encode($json);
    ...
    function state_autocomplete(){
        ...
        $stateObj=new States();
        $states = $stateObj->getStates($criteria);

        foreach ($states as $key => $state) {
            $json[] = array(
                'data' => $state['state_id'],
                'value'=> strip_tags(htmlentities($state['state_name'].' --State', ENT_QUOTES, 'UTF-8')),
                'type'=>'state',
                'label'=>strip_tags(htmlentities($state['state_name'], ENT_QUOTES, 'UTF-8'))
            );
        }
        
        $cityObj=new Cities();
        $cities = $cityObj->getCities($criteria);
        ...
        foreach ($cities as $key => $city) {
            ...
            $json[] = array(
                'data' => $city['city_id'],
                'value'=> strip_tags(htmlentities($city['city_name'].' --City', ENT_QUOTES, 'UTF-8')),
                'type'=>'city',
                'label'=> strip_tags(htmlentities($city['city_name'], ENT_QUOTES, 'UTF-8')),
            );
        ...
        $sort_order = array();
        foreach ($json as $key => $value) {
            $sort_order[$key] = $value['state_name'];
        }
        $arr["suggestions"]=$json;
        echo json_encode($arr);
//echo json_encode($json);
    ...
    function state_autocomplete_before_ci(){
        $post = Syspage::getPostedVar();
        $json = array();
        $criteria=array(
            "keyword"=>urldecode($post["keyword"]),
        ...
        foreach ($states as $key => $state) {
            $json[] = array(
                'data' => $state['state_id'],
                'value'      => strip_tags(htmlentities($state['state_name'], ENT_QUOTES, 'UTF-8'))
            );
        }
        $sort_order = array();
        foreach ($json as $key => $value) {
