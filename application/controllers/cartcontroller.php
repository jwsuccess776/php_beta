<?php
    ...
    class CartController extends CommonController
    {
        ...
        function checkout()
        {
            ...
            {
                $_SESSION['check_shop_id'] = $_GET['shop'];
                $checkout_shop_id = $_GET['shop'];
            }
            else
            {
                unset($_SESSION['check_shop_id']);

                Utilities::redirectUser(Utilities::generateUrl('cart'));
            }

            $r = $this->Cart->getProducts_($checkout_shop_id);
        ...
        {
            $this->set('products', $this->Cart->getProducts_($_GET['shop']));
            ...
            $this->set('cart_summary', $this->Cart->getCartFinancialSummary_($_GET['shop']));
        ...
        {
            $user = new User();
            $this->set('products', $cart_products = $this->Cart->getProducts_($_GET['shop']));
            $this->set('billing_address', $billing_address = $user->getUserAddress($this->Cart->getCartBillingAddress(), $this->getLoggedUserId()));
            $this->set('delivery_address', $delivery_address = $user->getUserAddress($this->Cart->getCartShippingAddress(), $this->getLoggedUserId()));
            $this->set('cart_summary', $cart_summary = $this->Cart->getCartFinancialSummary_($_GET['shop']));
        ...
        function checkout_sidebar_payment_summary()
        {
            ...
            $cart_products = $this->Cart->getProducts_($_GET['shop']);
            $cart_summary = $this->Cart->getCartFinancialSummary_($_GET['shop']);
            ...
            $this->set('cart_summary', $cart_summary);
            $this->set('user_balance', $user->getUserBalance($this->getLoggedUserId()));
            $this->set('payment_ready', $payment_ready);

            die(convertToJson(array('cart_count' => count($cart_products), 'html' => $this->_template->render(false, false, NULL, true))));
        }

        function checkout_payment_summary()
        {

            /* echo "NIK session<pre>";
              print_r($_SESSION);
              exit; */

            $mydb = Syspage::getdb();

            /* $payment_ready = $this->is_eligible_for_next_step(
              array(
              "products"=>true,
              "stock"=>true,
              "minimum"=>true,
            ...
            /* $cart_summary = $this->Cart->getCartFinancialSummary_($_GET['shop']);
              echo '$cart_summary ppp';
              p($cart_summary); */

            $payment_ready = 1; // for testing

            $paymentMethodObj = new PaymentMethods();

            $this->set('payment_methods', $payment_methods = $paymentMethodObj->getPaymentMethods(array("status" => 1)));
            ...
            if ($payment_ready)
            {
                ...
                $reference_number = substr($random . rand(100, 999), 10, 6);
                ...
                if ($this->Cart->hasShipping())
                {
                    ...
                    $order_data['order_shipping_city'] = $delivery_address["city_name"];//$delivery_address["ua_city"];
                ...
                $order_data['order_billing_city'] = $billing_address["city_name"];//$billing_address["ua_city"];
                ...
                $coupon_discount = $this->Cart->getCouponDiscounts();
                foreach ($this->Cart->getProducts_($_GET['shop']) as $cart_product)
                {

                    /* echo '<pre>';
                      print_r($cart_product);
                      exit; */
                    if ($cart_product['shop_id'] == $_GET['shop'])
                    {
                        //p($cart_product);
                        $product_coupon_discount = 0;
                        $product_coupon_discount = $coupon_discount['products'][$cart_product['product_id']];
                        ...
                        $order_data['products'][] = array(
                            ...
                            'discount_total' => $product_coupon_discount,
            ...
            $this->set('my_order_details', $order_info);
            die(convertToJson(array('cart_count' => count($cart_products), 'html' => $this->_template->render(false, false, NULL, true))));
        }
        ...
        function save_shipping_address_2_oct_2018()
        {
            $post = Syspage::getPostedVar();
            $json = array();
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                ...
                foreach ($cart_products as $key => $product_detail)
                {
                ...
                foreach ($tmp_cart_array as $classid => $products_arr)
                {
                    ...
                    {
                        $tmp_product_array[] = $cart_products[$productkey];
                        $cartval = $cart_products[$productkey];
                        $p_key = $cartval['key'];
                        //foreach($cart_products as $cartkey=>$cartval){
                        $sn++;
                        if ($cartval['shipping'])
                        {
                            if (!empty($cartval['shipping_class_id']) && $cartval['shipping_class_id'] != 0)
                            {
                                $shipping_address = $user->getUserAddress($this->Cart->getCartShippingAddress(), $this->getLoggedUserId());

                                $shipping_options = $product->getShippingClassById($cartval['product_id'], array("shipping_class_id" => $cartval["shipping_class_id"]));

                                $productKey = md5($p_key);
                                // p($post["shipping_locations"][md5($p_key)],0);
                                if (array_key_exists($post["shipping_locations"][md5($p_key)]))
                                {

                                    //echo 'niknioknik';
                                    if (empty($post["shipping_locations"][md5($p_key)]) || (!in_array($post["shipping_locations"][md5($p_key)], array_column($shipping_options, 'pship_id'))))
                                    {
        ...
        function save_shipping_address_old()
        {
            $post = Syspage::getPostedVar();
            $json = array();
            if ($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                ...
                foreach ($cart_products as $key => $product1)
                {
                    $product_shipping_class_id[$key] = $product1['shipping_class_id'];
                }
                //echo 'product_shipping_class_id'; p($product_shipping_class_id);
                foreach ($cart_products as $key => $product_detail)
                {
                    //$product_shipping_class_id[$key] = $product1['shipping_class_id'];
                    $shipping_class_id = $product_detail["shipping_class_id"] == 0 ? $product_detail["key"] : $product_detail["shipping_class_id"];
                    ...
                    foreach ($products_arr as $key => $productkey) {
                        ...
                        if ($cartval['shipping']) {
                            if (!empty($cartval['shipping_class_id']) && $cartval['shipping_class_id'] != 0) {
                                ...
                                //p($post["shipping_locations"][md5($p_key)],0);
    //                            if (array_key_exists($post["shipping_locations"][md5($p_key)])) {
                                if (array_key_exists(md5($p_key), $post["shipping_locations"])) {
                                    //echo 'niknioknik';
                                    //p($post['shipping_locations'][md5($p_key)]);
                                    ...
                                    if (empty($post["shipping_locations"][md5($p_key)]) || (!in_array($post["shipping_locations"][md5($p_key)], array_column($shipping_options, 'pship_id')))) {
                                        ...
                                        $json['error']['product'][md5($p_key)] = sprintf(Utilities::getLabel('M_Shipping_Info_Required'), htmlentities($cartval['name']));
                                        //$json['error']['product'][$sn] = sprintf(Utilities::getLabel('M_Shipping_Info_Required'), htmlentities($cartval['name']));
                                    }
                                }
                                //  die('nik test tse df');
                            } else {
                                if (empty($cartval['shipping_free'])) {
                                    $shipping_address = $user->getUserAddress($this->Cart->getCartShippingAddress(), $this->getLoggedUserId());
                                    //p($cartval['product_id']);
                                    $shipping_options = $product->getProductShippingRates($cartval['product_id'], array("country" => $shipping_address["ua_country"]));
                                    $productKey = md5($cartval["key"]);
                                    if (empty($post["shipping_locations"][md5($cartval["key"])]) || (!in_array($post["shipping_locations"][md5($cartval["key"])], array_column($shipping_options, 'pship_id')))) {
        ...
        function is_eligible_for_next_step($criteria = array(), $redirect = true, $shop_id = "")
        {
            $error_set = false;
            foreach ($criteria as $key => $val)
            {
                switch ($key)
                {
                    ...
                    case 'self_product':
                        ...
                        break;
                    case 'minimum':
                        $products = $this->Cart->getProducts_($shop_id);
                        foreach ($products as $product)
                        {
                            if ($product['minimum'] > $product["quantity"])
                            {
                    ...
                    case 'shipping_options':
                        if ((!$this->Cart->hasShippingOptionSet($shop_id)) && ($this->Cart->hasShipping($shop_id)))
                        {
                            $error_set = true;
