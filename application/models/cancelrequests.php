...
class CancelRequests extends Model {
	...
	function search_cancel_requests($criteria, $count='') {
       $srch = new SearchBase('tbl_order_cancel_requests', 'tpcr');
		...
		//Ashish - Changed join to get correct buyer details in case of cancellation request from seller
		$srch->joinTable('tbl_users', 'INNER JOIN', 'tord.order_user_id=tb.user_id', 'tb');
		...
		$srch->addMultipleFields(array('tpcr.*','tb.*','torp.*','tord.order_id','trr.*','tors.orders_status_name','tb.user_name as buyer_name','tb.user_email as buyer_email','tv.user_name as vendor_name','tv.user_email as vendor_email','ts.shop_user_id as vendor_id'));
        ...
        foreach($criteria as $key=>$val) {
            ...
            switch($key) {
					...
					case 'return':
						...
						elseif ($val=="received"){
							$cnd=$srch->addCondition('tv.user_id', '=', intval($user));
							//Ashish - Added condition to get only requests that are not made by vendor
							$cnd->attachCondition('tpcr.cancellation_request_user_id', '!=',$user,'AND');
						}
					break;
