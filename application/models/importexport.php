...
class Importexport extends Model {
    ...
    function isUploadedFileValidFileXML($files) {
        $valid_arr = array(
            'application/xml',
            'text/xml',
    ...
    function setLocalfile(&$localfile, $dirimg) {
        ...
        if ($nrimg[0] === '') {
            ...
            $imgnm = str_replace($nrimg[0], $nrimg[0] + 1, $imgnm);
        }
        $localfile = $dirimg . $imgnm;
        if (file_exists($dirimg . $imgnm)) {
            return $this->setLocalfile($localfile, $dirimg);
        }
        return $imgnm;
    }
    ...
    function upload_data($filename, $incremental, $attachment, $shop_user_id = 0) {
        
        //$path = Utilities::getFilepathOnDirectory() . Utilities::getCurrUploadDirPath('importexport/');
    ...
    function get_xml_data($fullpath){
        $xml=simplexml_load_file($fullpath);
...
//		}
    ...
    protected function getProductFeatured() {
        $query = $this->db->query("SELECT prod_id, prod_featuered FROM `tbl_prod_details`");
        $res = $this->db->fetch_all($query);
        $featuered_prods = array();
        foreach ($res as $row) {
            $product_id = $row['prod_id'];
            $featuered = $row['prod_featuered'];
            $featuered_prods[$product_id] = $featuered;
        }
        return $featuered_prods;
    }
    ...
    protected function storeProductIntoDatabase(&$product, &$product_fields, &$available_store_ids, &$weight_class_ids, &$length_class_ids, &$url_alias_ids, &$userProductIds, &$shop_user_id, &$available_seller_prod_ids) {
        ...
        $assignField['prod_view_count'] = isset($product['prod_view_count']) ? $product['prod_view_count'] : 0;
        $assignField['prod_sold_count'] = isset($product['prod_sold_count']) ? $product['prod_sold_count'] : 0;
        ...
        $productDetails['prod_featuered'] = $product['prod_featuered'];
    ...
    protected function storeAdditionalImageIntoDatabase(&$image, &$old_product_image_ids, $saveToTempTable = false) {
        ...
        if (isset($old_product_image_ids[$product_id][$image_name])) {
            ...
            if ($saveToTempTable == true) {
                $imgArr['image_downloaded'] = 0;
            }
    ...
    protected function uploadProducts(&$reader, $incremental, &$available_product_ids = array(), $shop_user_id = 0, &$available_seller_prod_ids = array()) {
        ...
        $featuered_prods = $this->getProductFeatured();
        ...
        for ($i = 0; $i < $k; $i += 1) {
            ...
            if ($shop_user_id > 0) {
                ...
                if ($shopUsers[$prod_shop] != $prod_added_by || $shop_user_id != $prod_added_by) {
                    continue;
                }
            } else {
                // Action perfomed by manager
                if ($shopUsers[$prod_shop] != $prod_added_by) {
                    continue;
                }
            }
            ...
            if (!$incremental) {
                $product['prod_sold_count'] = intval($prod_sold_count);
                $product['prod_view_count'] = intval($prod_view_count);
                $product['prod_featuered'] = 0;
            } else {
                ...
                $product['prod_featuered'] = isset($featuered_prods[$prod_id]) ? $featuered_prods[$prod_id] : 0;
            }
    ...
    protected function uploadProductSkuStock(&$reader, $incremental, $shop_user_id) {
        ...
        for ($i = 0; $i < $k; $i += 1) {
            ...
            if (!in_array($product_id, $userProductIds)) {
                continue;
            }
