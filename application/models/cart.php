<?php
    ...
    class Cart extends Model
    {
        ...
        {
            //echo "<pre>";
            //echo 'cart_user_id : '.$this->cart_user_id.'<br/>';
            if (isset($this->cart_user_id))
            {
        ...
        public function getProducts_($chek_out_id)
        {

            if (!$this->data)
            {
//******************************************************************
                $tmp_shipping_class_ids_arr = array();
                foreach ($this->SYSTEM_ARR['cart'] as $key => $quantity)
                {
                    ...
                    if ($shipping_id > 0)
                    {
                        $shipping_class_id = $product_info['shipping_class_id'];
                        $tmp_shipping_class_ids_arr[$shipping_class_id] = $shipping_id;
                    }
                }
                
//p($tmp_shipping_class_ids_arr);
//***********************************************************************
                foreach ($this->SYSTEM_ARR['cart'] as $key => $quantity)
                {
                    $product = unserialize(base64_decode($key));
                    ...
                    if ($product_info)
                    {
                        ...
                        if (!empty($product_info['shipping_class_id']) && $product_info['shipping_class_id'] != 0)
                        {
                            $shipping_options_states = $pObj->getShippingClassById($product_info['prod_id'], array("shipping_class_id" => $product_info["shipping_class_id"], "state" => $shipping_address["ua_state"]), FALSE);
                            $shipping_options_cities = $pObj->getShippingClassById($product_info['prod_id'], array("shipping_class_id" => $product_info["shipping_class_id"], "city" => $shipping_address["ua_city"]), FALSE);
                            
                            if (empty($shipping_options_states) && empty($shipping_options_cities))
                            {
                                $shipping_options = $pObj->getShippingClassById($product_info['prod_id'], array("shipping_class_id" => $product_info["shipping_class_id"], "state" => $shipping_address["ua_state"]));
                            }
                            else {
                                $shipping_options=array_merge($shipping_options_states,$shipping_options_cities);     
                            }
                        }
                        else
                        {
                            $shipping_options_states = $pObj->getProductShippingRates($product_info['prod_id'], array("country" => $shipping_address["ua_country"], "state" => $shipping_address["ua_state"]), FALSE);
                            
                            $shipping_options_cities = $pObj->getProductShippingRates($product_info['prod_id'], array("country" => $shipping_address["ua_country"], "city" => $shipping_address["ua_city"]), FALSE);
                            
                            if (empty($shipping_options_states) && empty($shipping_options_cities))
                            {
                                $shipping_options = $pObj->getProductShippingRates($product_info['prod_id'], array("country" => $shipping_address["ua_country"], "state" => $shipping_address["ua_state"]));
                               // p($shipping_options,0);
                            }
                            else{
                                //p($shipping_options,0);
                                //p($shipping_options_cities,0);                                
                                $shipping_options=array_merge($shipping_options_states,$shipping_options_cities);
                                
                            }
                            //$shipping_options = $pObj->getProductShippingRates($product_info['prod_id']);
                        }
                        ...
                        //***************************  Overwrite   $shipping_id  *******************
                        else
                        {
                            $getshipping_class_id = $product_info['shipping_class_id'];
                            if ($shipping_id == 0 && $product_info['shipping_class_id'] != 0)
                            {
                                $shipping_id = $tmp_shipping_class_ids_arr[$getshipping_class_id];
                                $selected_shipping_option = $shipping_options[$shipping_id];
                                $shipping_cost = $product_info["prod_ship_free"] == 0 ? $selected_shipping_option["pship_additional_charges"] * ($quantity) : 0;
                            }
                        }
                        ...
                        /* echo 'commission_percentage : '.$commission_percentage;
                        ...
                        if ($product_info['prod_shop'] == $chek_out_id)
                        {
                            $this->data[$key] = array(
                                'key' => $key,
                                ...
                                'shipping' => $product_info['prod_type'] == 1 ? $product_info['prod_requires_shipping'] : 0,
            ...
            //echo 'this data'; p($this->data);
            return $this->data;
        }
        public function getProducts_before_city($chek_out_id)
        {
...
//***********************************************************************
                foreach ($this->SYSTEM_ARR['cart'] as $key => $quantity)
                {
                    ...
                    if ($product_info)
                    {
                        ...
                        if (!empty($product_info['shipping_class_id']) && $product_info['shipping_class_id'] != 0)
                        {
                            $shipping_options = $pObj->getShippingClassById($product_info['prod_id'], array("shipping_class_id" => $product_info["shipping_class_id"], "state" => $shipping_address["ua_state"]), FALSE);
                            if (empty($shipping_options))
                            {
                                $shipping_options = $pObj->getShippingClassById($product_info['prod_id'], array("shipping_class_id" => $product_info["shipping_class_id"], "state" => $shipping_address["ua_state"]));
                            }
                        ...
                        {
                            $shipping_options = $pObj->getProductShippingRates($product_info['prod_id'], array("country" => $shipping_address["ua_country"], "state" => $shipping_address["ua_state"]), FALSE);
                            if (empty($shipping_options))
                            {
                                $shipping_options = $pObj->getProductShippingRates($product_info['prod_id'], array("country" => $shipping_address["ua_country"], "state" => $shipping_address["ua_state"]));
                            }
                ...
                foreach ($this->SYSTEM_ARR['cart'] as $key => $quantity)
                {
                    ...
                    if ($product_info)
                    {
                        ...
                        $this->data[$key] = array(
                            ...
                            'shipping' => $product_info['prod_type'] == 1 ? $product_info['prod_requires_shipping'] : 0,
                            'image' => $product_info['image_file'],
                            'imported_image' => $product_info['prod_image'],
                            'option' => $option_data,
                            'shipping_options' => $shipping_options,
                            'shipping_id' => $shipping_id,
                            'selected_shipping_option' => $selected_shipping_option,
        ...
        public function getCartFinancialSummary_($chek_out_id)
        {
            ...
            $products = $this->getProducts_($chek_out_id);

            $itemcount = 0;
            $products_qty_count = 0;
            ...
            foreach ($products as $key => $product)
            {
                ...
                {
                    $products_qty_count[$product['shipping_id']] += $product['quantity'];
                    $product_shipping_id[$product['product_id']] = $product['shipping_id'];
                }

                if ($product['shop_id'] == $chek_out_id)
                {
                    $selected_shipping_option_data[$product['shipping_id']] = $product['selected_shipping_option'];
                    $selected_shipping_option_data[$product['shipping_id']]['shipping_price'] = $product['shipping_price'];
            ...
            //shipping changes
            //Comment On 9 Feb 2018 10:18am  Start
            /* if(!empty($selected_shipping_option_data))
              {
              foreach($selected_shipping_option_data as $idx => $shipping_option)
              {
              $cart_shipping_total += $shipping_option['shipping_price'];
              if($products_qty_count[$idx] >1)
              {
              for($i = 1; $i < $products_qty_count[$idx]; $i++)
              {
              $cart_shipping_total += $shipping_option['pship_additional_charges'];
              }
              }
              }
              }
              $net_total_without_discount += $cart_shipping_total; */
            //Comment On 9 Feb 2018 10:18am   End
//               echo $cart_shipping_total;exit;

            /* $checkShippingClassId[$key] = $product['shipping_id'];
            ...
            /* echo 'getCartFinancialSummary_';
              p($cart_summary); */
            //echo 'hhhhhh---'.$cart_shipping_total;
            //$cart_summary1 = array('nik'=>$cart_shipping_total);
            return $cart_summary;
        }
        ...
        public function getCouponDiscounts()
        {
            ...
            if ($coupon_info)
            {
                ...
                foreach ($this->getProducts() as $product)
                {
                    ...
                    $product_data[$product['product_id']] = round($discount, 2);
                }
                ...
                if ($discount_total > $coupon_info['coupon_max_discount_value'])
                {
                    ...
                    $increment_excl_prod_total = $this->calculateExcludedProdsTotal($excluded_products);
                    foreach ($this->getProducts() as $product)
                    {
                        ...
                        if ($status)
                        {
                            $product_max_discount = round(((($product['total'] + $increment_excl_prod_total) / $cart_sub_total) * $discount_total), 2);
                        }
                        $product_data[$product['product_id']] = $product_max_discount;
                    }
                }
        ...
        public function calculateExcludedProdsTotal($excl_items)
        {
            $couponObj = new Coupons($this->cart_user_id);
            $coupon_info = $couponObj->getCoupon(self::getCartDiscountCoupon());
            $increment_excl_prod_total = 0;
            foreach ($this->getProducts() as $product)
            {
                ...
                if (!$status)
                {
                    $increment_excl_prod_total += $product['total'] / ($total_products - $excl_items);
                }
            }
            return $increment_excl_prod_total;
        }
        ...
        public function hasShipping($shop = '0')
        {
            if ($shop != 0)
            {
                foreach ($this->getProducts_($shop) as $product)
                {
                    if ($product['shipping'])
                    {
                        return true;
        ...
        public function hasShippingOptionSet($shop = '0')
        {
            $shipping_selected = true;
            $prod = $this->getProducts_($shop);
            foreach ($prod as $product)
            {
                if ($product['shop_id'] == $shop)
                {
                    if ($product['shipping'])
                    {
                        $pObj = new Products();
        ...
        public function setProductsShipping($arr)
        {
            foreach ($this->SYSTEM_ARR['cart'] as $key => $quantity)
            {
                ...
                if ($arr["shipping_locations"][md5($key)] >= 0)
                {
                    ...
                    if (isset($arr["shipping_locations"][md5($key)]) && $arr["shipping_locations"][md5($key)] == 0 && !empty($arr["shipping_services"][md5($key)]))
                    {
        ...
        public function setProductsShipping_nik($arr)
        {
        ...
        function order_invoice_format()
        {
            ...
            if (!$this->db->update_from_array('tbl_configurations', array('conf_val' => $new_value), array('smt' => 'conf_var = ? ', 'vals' => array('CONF_INVOICE_FORMAT'))))
            {
