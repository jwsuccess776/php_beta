<?php
    ...
    class Orders extends Model
    {
        ...
        function addUpdateCustomerOrder($data)
        {
            ...
            $values = array(
                ...
                'order_site_commission_static' => $data['order_site_commission'],
            ...
            if (isset($data['products']) && is_array($data['products']) && sizeof($data['products']) > 0)
            {
                foreach ($data['products'] as $product_id => $product)
                {
                    ...
                    $values = array(
                        ...
                        'opr_discount_total' => $product['discount_total'],
                        ...
                        'opr_commission_charged_static' => $product['commission'],
                        'opr_commission_percentage' => $product["commission_percentage"],
                        'opr_commission_percentage_static' => $product['commission_percentage'],
                        ...
                        'opr_shipping_required' => $product["product_type"] == 1 ? $product["shipping_required"] : 0,
        ...
        function getOrderBillingShippingAddress($order_id = 0)
        {
            ...
            $address["shipping"] = "<strong>" . $row["order_shipping_name"] . "</strong><br/>" . $row["order_shipping_address1"] . $address2 . "<br/>" . $row["order_shipping_city"] . ", " . $row["order_shipping_state"] . " - " . $row["order_shipping_postcode"] . "<br />" . $row["order_shipping_country"] . $phone;
            return $address;
        }
        ...
        function getSubOrderDetail($oid)
        {
            ...
            $str .= '</table><br/><br/><table cellspacing="0" cellpadding="0" border="0" width="100%" style="border:1px solid #ddd; border-collapse:collapse;"><tr><td style="padding:10px;background:#eee;font-size:13px;border:1px solid #ddd; color:#333; font-weight:bold;"  bgcolor="#f0f0f0">' . Utilities::getLabel('L_Order_Billing_Details') . '</td><td style="padding:10px;background:#eee;font-size:13px;border:1px solid #ddd; color:#333; font-weight:bold;" bgcolor="#f0f0f0"><strong>' . Utilities::getLabel('L_Order_Shipping_Details') . '</strong></td></tr><tr><td valign="top" style="padding:10px;font-size:13px; color:#333;border:1px solid #ddd;" >' . $vendor_order_detail["address"]["billing"] . '</td>';
        ...
        public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false)
        {
            ...
            if ($order_info)
            {
                if (!$this->db->update_from_array('tbl_orders', array('order_status' => (int) $order_status_id, 'order_date_updated' => date('Y-m-d H:i:s')), array('smt' => 'order_id = ? ', 'vals' => array((int) $order_info["order_id"])), true))
                {
                    $this->error = $this->db->getError();
                    return false;
                }
        ...
        public function addOrderPaymentHistory($order_id, $order_payment_status, $comment = '', $notify = false)
        {
            ...
            if ($order_info)
            {
                if (!$this->db->update_from_array('tbl_orders', array('order_payment_status' => (int) $order_payment_status, 'order_date_updated' => "now()"), array('smt' => 'order_id = ? ', 'vals' => array((int) $order_id))))
                {
                    $this->error = $this->db->getError();
                    return false;
                }
        ...
        public function addChildOrderHistory($child_order_id, $opr_order_status_id, $comment = '', $notify = false, $tracking_number, $release_payments = 0,$cancellation_request=null,$buyer_cancellation_comments=null)
        {
            $p = new Products();
            $child_order_info = $this->getOrderProductsByOprId($child_order_id);
            if ($child_order_info)
            {
                if (!$this->db->update_from_array('tbl_order_products', array('opr_status' => (int) $opr_order_status_id), array('smt' => 'opr_id = ? ', 'vals' => array((int) $child_order_id))))
                {
                    $this->error = $this->db->getError();
                    return false;
                }
                ...
                if (($child_order_info['opr_status'] != Settings::getSetting("CONF_DEFAULT_CANCEL_ORDER_STATUS")) && ($opr_order_status_id == Settings::getSetting("CONF_DEFAULT_CANCEL_ORDER_STATUS")) && ($child_order_info["order_payment_status"] == 1))
                {
                    ...
                    $txn_amount = $child_order_info['opr_net_charged'] - $child_order_info['opr_discount_total'];
                    if ($txn_amount > 0)
                    {
						...
						//Ashish - added charge commission and payment gateway fee or only payment gateway fee if seller request approved by admin
						
						//Check if we need to debit seller for charging fee or payment processing fee or both
						if($cancellation_request != null)		//This is seller cancellation request approved by admin
						{
							$txn_amount = 0;
							if($cancellation_request=="charged_for_processing_fee_and_commission")
							{
								$txn_amount = $child_order_info['opr_commission_charged'] - $child_order_info['opr_refund_commission'];
							}
							else if($cancellation_request=="chargeed_for_processing_fee_only")
							{
								$txn_amount = $child_order_info['order_payment_gateway_charges'];
							}
							if($txn_amount > 0)
							{
								//$comments = "Cancellation Request :: " . $cancellation_request; 
								$comments = sprintf(Utilities::getLabel('M_Charged_Commission_for_order'), $formatted_request_value);
                ...
                if (!in_array($child_order_info['opr_status'], (array) Settings::getSetting("CONF_DEFAULT_SHIPPING_ORDER_STATUS")) && in_array($opr_order_status_id, (array) Settings::getSetting("CONF_DEFAULT_SHIPPING_ORDER_STATUS")) && ($child_order_info["order_payment_status"] == 1))
                {
                    ...
                    $this->db->update_from_array('tbl_order_products', array('opr_shipped_date' => date('Y-m-d H:i:s')), array('smt' => 'opr_id = ? ', 'vals' => array((int) $child_order_id)), true);
                }


                // If current order status is not completed but new status is completed then commence completing the order
                if (!in_array($child_order_info['opr_status'], (array) $this->getVendorOrderPaymentCreditedStatuses()) && in_array($opr_order_status_id, (array) $this->getVendorOrderPaymentCreditedStatuses()) && (in_array($child_order_info["order_payment_status"], array(1, 2))) && ($child_order_info['PA_TXN_STS'] != 0 || is_null($order_info['PA_TXN_STS'])) || (in_array($opr_order_status_id, (array) $this->getVendorOrderPaymentCreditedStatuses()) && ($child_order_info["order_payment_status"] == 1) && $release_payments)
                )
                {
                    ...
                    $this->db->update_from_array('tbl_order_products', array('opr_completion_date' => date('Y-m-d H:i:s')), array('smt' => 'opr_id = ? ', 'vals' => array((int) $child_order_id)), true);
                ...
                if (in_array($opr_order_status_id, (array) Settings::getSetting("CONF_BUYER_ORDER_STATUS")) && $notify)
                {
					...
					{
						$emailNotificationObj = new Emailnotifications();
						$emailNotificationObj->Seller_Cancellation_Approved_Order_Status_Update_Buyer($comment_id,$cancellation_request,$buyer_cancellation_comments);
					}
					else
					{
						$emailNotificationObj = new Emailnotifications();
	...
	<td width="20%" style="padding:10px;background:#eee;font-size:13px; border:1px solid #ddd;color:#333; font-weight:bold;" align="right">' . Utilities::getLabel('L_Total') . '</td>
            ...
            //p($shipping_address, 0);
            //p($order_details);

            $shipping_address = $order_details['order_shipping_required'] ? $order_details["addresses"]["shipping"] : '-NA-';
            $str .= '</table><br/><br/><table cellspacing="0" cellpadding="0" border="0" width="100%" style="border:1px solid #ddd; border-collapse:collapse;"><tr><td style="padding:10px;background:#eee;font-size:13px;border:1px solid #ddd; color:#333; font-weight:bold;"  bgcolor="#f0f0f0">' . Utilities::getLabel('L_Order_Billing_Details') . '</td><td style="padding:10px;background:#eee;font-size:13px;border:1px solid #ddd; color:#333; font-weight:bold;" bgcolor="#f0f0f0"><strong>' . Utilities::getLabel('L_Order_Shipping_Details') . '</strong></td></tr><tr><td valign="top" style="padding:10px;font-size:13px; color:#333;border:1px solid #ddd;" >' . $order_details["addresses"]["billing"] . '</td>';
            $str .= '<td style="padding:10px;font-size:13px; color:#333;border:1px solid #ddd;">' . $shipping_address . '</td></tr></table>';
            return $str;
        }
        ...
        function getBuyerAllowedOrderCancellationStatuses()
        {
             ...
             */
            /* echo '<pre>';
              print_r($buyer_allow_cancel_statuses);
              exit; */
        ...
        function getBuyerAllowedOrderReturnStatuses()
        {
            ...
            //print_r($buyer_allow_return_statuses);
            //exit;
