...
class Categories extends Model{
	...
	function getCategoriesHavingRecords($type) {
        ...
        //$srch->addCondition('tc.category_parent', '=',0); // add & remove/comment by NIK
     ...
     * @return array
     */
    function getCategoriesAssocArrayFront_new($parent_id=0, $type=1, $pagesize=100) {
        $db = &Syspage::getdb();

        $selected_categories = Settings::getSetting("CONF_MENU_CATEGORY_SELECT");
        $sel_cate_id = array();
       ...
       /* if(!empty($parent_id))
        {
            $sel_cate_id[$parent_id] = $parent_id;
        }*/

		$srch = new SearchBase('tbl_categories', 'tc');
		$srch->addCondition('tc.category_is_deleted', '=',0);

       // $srch->addCondition('tc.category_id', 'IN',$sel_cate_id);
        ...
        $query = "SELECT tc.*,IFNULL(tcs.total_sub_cats,0) as subcats FROM tbl_categories tc LEFT JOIN (".$qry_subcats_received.") tcs on tcs.category_parent=tc.category_id WHERE category_status=1 and category_is_deleted=0 and category_type='{$type}'";

        if(!empty($parent_id))
        {
            $query .= ' and tc.category_parent='.$parent_id.'';
        }

        if(empty($parent_id))
        {
            $query .= ' and tc.category_id IN ('.implode(",",$sel_cate_id).')';
        }
    ...
    function getCategoriesAssocArrayFront($parent_id=30, $type=1, $pagesize=100) {
        $db = &Syspage::getdb();
	...
	function getCategoriesAssocArrayFrontOneLevel($parent_id=30, $type=1, $pagesize=100) {
        $db = &Syspage::getdb();
	...
	function getProductCategoriesHavingRecordsOneLevel($parent,$shop) {
		$categories=Categories::getCategoriesAssocArrayFrontOneLevel($parent,1);
