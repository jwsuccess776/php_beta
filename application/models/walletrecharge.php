...
class WalletRecharge {
	...
	function createUniqueReferanceNumber()
	{
		...
		$srch->addCondition('trwr.rwr_referance_no', '=', intval($reference_number)); 
		//echo "here";
		//echo ($srch->getquery());
		...
		{
			$this->createUniqueReferanceNumber();
		}else{
	...
	function saveRechargeRequest($data){
		...
		/*$srch = new SearchBase('tbl_recharge_wallet_requests', 'trwr');
		$srch->addCondition('trwr.rwr_referance_no', '=', intval('178892')); 
		$rs = $srch->getResultSet();
		$this->total_records = $srch->recordCount();
		
		echo 'total_records'.$this->total_records;
		exit;
			*/
		$assign_fields = array(
		...
		//print_r($assign_fields);
					
		$assign_fields['rwr_referance_no']=$this->createUniqueReferanceNumber();
	...
	function getWalletRequests($criteria=array()){
		...
		{	
			$_rows=$this->db->fetch($rs);
		//	$random = strtoupper(uniqid());
		//	$reference_number =  substr($random . rand(100, 999), 10, 6);
		//	$_rows['recharge_unique_referance_no']=$reference_number;
			
		//	echo $reference_number;
			
		}else
		{
			$_rows=$this->db->fetch_all($rs);
		}
		$rows=$this->db->fetch($rs);
		/*echo "<pre>";
		print_r($_rows);
		echo "</pre>";
		exit;*/
