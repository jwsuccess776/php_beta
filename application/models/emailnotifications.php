<?php
    ...
    class Emailnotifications extends Model
    {
            ...
            if ($order_detail)
            {
                ...
                foreach ($order_vendors as $key => $val):
                    ...
                    $arr_replacements = array(
                        ...
                        '{order_status_name}'=>$val['orders_status_name'] //Added On 16-09-2018
        ...
        function Order_Status_Update_Buyer($comment_id)
        {
            ...
            if ($order_comment && $order_comment["customer_notified"])
            {
                ...
                $arr_replacements = array(
                    ...
                    '{shipment_information}' => "<br/><br/>" . $shipment_information,
                    '{sellar_contacts}'  => "<br/><br/>".$order_comment['opr_shop_owner_name']."<br/>".$order_comment['opr_shop_owner_email']."<br/>".$order_comment['opr_shop_owner_phone']
                );
                Utilities::sendMailTpl($order_comment["order_user_email"], "child_order_status_change", $arr_replacements);
                return true;
            }
        ...
        function Seller_Cancellation_Approved_Order_Status_Update_Buyer($comment_id,$cancellation_request,$buyer_cancellation_comments)
        {
            $orderObj = new Orders();
            $order_comment = $orderObj->getOrderComments(array("id" => $comment_id), 1);
            if ($order_comment && $order_comment["customer_notified"])
            {
                ...
                $arr_replacements = array(
                    ...
                    '{invoice_number}' => $order_comment["opr_order_invoice_number"],
                    '{order_items_table_format}' => $order_items_table_format,
                    '{order_admin_comments}' => "<strong>Comments:</strong>" . $buyer_cancellation_comments, //$cancellation_request['cancellation_request_message'],
                    '{shipment_information}' => "<br/><br/>" . $shipment_information,
                    '{sellar_contacts}'  => "<br/><br/>".$order_comment['opr_shop_owner_name']."<br/>".$order_comment['opr_shop_owner_email']."<br/>".$order_comment['opr_shop_owner_phone']
                );
                Utilities::sendMailTpl($order_comment["order_user_email"], "child_order_status_change", $arr_replacements);
        ...
        function SendSellerOrderCancellationNotification($cancellation_requeest)
        {
            $crObj = new CancelRequests();
            $cancellation_request = $crObj->getCancelRequest($cancellation_requeest);
            if ($cancellation_request)
            {
                ...
                Utilities::sendMailTpl($cancellation_request["vendor_email"], "vendor_order_cancellation_notification_vendor", $arr_replacements);
                $arr_replacements["{user_name}"] = "Admin";
                Utilities::sendMailTpl(Settings::getSetting("CONF_ADMIN_EMAIL"), "vendor_order_cancellation_notification_admin", $arr_replacements);
                $emails = explode(',', Settings::getSetting("CONF_ADDITIONAL_ALERT_EMAILS"));
                foreach ($emails as $email)
                {
                    if (Utilities::utf8_strlen($email) > 0 && preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $email))
                    {
                        Utilities::sendMailTpl($email, "vendor_order_cancellation_notification_admin", $arr_replacements);
                    }
                }
                return true;
            }
        ...
        function SendSellerCancellationRequestUpdateNotification($cancel_request_id)
        {
            global $status_arr;
            $crObj = new CancelRequests();
            $cancellation_request = $crObj->getCancelRequest($cancel_request_id);
            if ($cancellation_request)
            {
                ...
                $arr_replacements = array(
                    ...
                    '{invoice_number}' => $cancellation_request["opr_order_invoice_number"],
                    '{request_status}' => $status_arr[$cancellation_request['cancellation_request_status']],
                    '{user_name}' => htmlentities($cancellation_request['vendor_name']),
                );
                Utilities::sendMailTpl($cancellation_request["vendor_email"], "cancellation_request_approved_declined", $arr_replacements);
                return true;
            }
            else
            {
        ...
        function timer_reach_2_days_alert($order_id)
        {
            global $payment_status_arr;
            $orderObj = new Orders();
            $order_detail = $orderObj->getOrderById($order_id);
            if ($order_detail)
            {
                $arr_replacements = array(
                    ...
                    '{user_full_name}' => htmlentities(trim($order_detail['order_user_name'])),
                    '{invoice_number}' => $order_detail['order_invoice_number'],
                    '{order_payment_status}' => $payment_status_arr[$order_detail['order_payment_status']],
                );

               //@23may Utilities::sendMailTpl(Settings::getSetting("CONF_ADMIN_EMAIL"), "timer_reach_2_days_alert_buyer", $arr_replacements);
                ...
                foreach ($emails as $email)
                {
                    if (Utilities::utf8_strlen($email) > 0 && preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $email))
                    {
                        //@23mayUtilities::sendMailTpl($email, "timer_reach_2_days_alert_admin", $arr_replacements);
                    }
                }

                Utilities::sendMailTpl($order_detail["order_user_email"], "timer_reach_2_days_alert_buyer", $arr_replacements);
            }
            return true;
        }
