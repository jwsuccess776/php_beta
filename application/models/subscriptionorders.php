...
//		echo($srch->getquery()."<br/><br/>");
	...
	function getWalletOrders($criteria) {
        foreach($criteria as $key=>$val) {
        	if(strval($val)=='') continue;
				$add_criteria[$key] = $val;
		}
        $srch = self::search1($add_criteria);
		//echo($srch->getquery()."<br/><br/>");
	...
	function search1($criteria, $count='') {
		
		$srch = new SearchBase( 'tbl_recharge_wallet_requests', 'trwr' );
		$srch->joinTable('tbl_users', 'LEFT OUTER JOIN', 'trwr.rwr_member_id = tu.user_id', 'tu');
