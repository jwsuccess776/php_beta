<?php
    ...
    class Products extends SearchBase
    {
        ...
        /* public function joinWithShippingClassTable(){
          $this->joinTable('tbl_shippingclass', 'LEFT JOIN', 'tp.shipping_class_id=sc.id', 'sc');
          } */
   ...
   SUM(IF(`pclick_datetime`>CURRENT_DATE - INTERVAL 1 MONTH,`pclick_cost`,0)) monthly_cost",
            ...
            foreach ($conditions as $key => $val)
            {
                ...
                switch ($key)
                {
                    ...
                    case 'shipping_class_id':
                        $this->addCondition('tp.shipping_class_id', '=', intval($val));
                        break;
        ...
        function check_assion_class($class_id)
        {
            $class_id = intval($class_id);
            if ($class_id > 0 != true)
                return array();
            $criteria['shipping_class_id'] = $class_id;
        ...
        $assign_fields['shipping_class_id'] = intval($data['class_name']);
        ...
        if (!$this->db->update_from_array('tbl_product_images', array('image_prod_id' => $this->getProdId()), array('smt' => 'image_prod_id = ? and image_session = ?', 'vals' => array(0, session_id())))) {
            $this->error = $this->db->getError();
            return false;
        }
        ...
        foreach ($data["product_shipping"] as $key => $val):
            ...
            if($val['type']=='city'){
                 $state=$cityObj->getData($val['state_id']);                        
            }elseif($val['type']=='state'){
                $state = $stateObj->getData($val["state_id"]);
            } else {
                $state=FALSE;
            }
            ...
            if (($state || $val["state_id"] == -1) && $shipping_company && $shipping_duration) {
                $info["pship_prod_id"] = $this->getProdId();
                $info["pship_id"] = (int) $val["pship_id"];
                //$info["pship_country"]=(int)$val["country_id"];
                if($val['type']=='state'){
                   $info["pship_state"] = (int) $val["state_id"];
                   $info["pship_city"]='-1';
                }
                elseif($val['type']=='city'){
                   $info["pship_state"]=0; 
                   $info["pship_city"] = (int) $val["state_id"];                    
                }
        ...
        function addUpdateProduct_before_city($data)
        {
            $product_id = intval($data['prod_id']);
    ...
    function getShippingClassById($product_id, $criteria = array(), $include_everywhere = TRUE) {
        ...
        //
        $srch->joinTable('tbl_shippingclass', 'LEFT JOIN', 'tpsr.pship_shipping_class_id=shipping_class.id and shipping_class.ship_class_is_deleted=0', 'shipping_class');

        $srch->joinTable('tbl_states', 'LEFT JOIN', 'tpsr.pship_state=tstate.state_id and tstate.state_delete=0', 'tstate');
        $srch->joinTable('tbl_cities', 'LEFT JOIN', 'tpsr.pship_city=tci.city_id and tci.city_delete=0', 'tci');
        ...
        //$srch->addOrder('(`pship_state` = -1),state_name');
        $srch->addOrder('FIELD (`pship_state`,"-1") DESC,`pship_id`');


        $srch->addMultipleFields(array('tpsr.*', 'tc.*', 'tsc.*', 'tsd.*', 'tpd.prod_ship_free', 'shipping_class.class_name', 'tstate.*','tci.city_name'));
        ...
        foreach ($criteria as $key => $val) {
            switch ($key) {
                case 'id':
                    ...
                    break;
                case 'shipping_class_id':
                    $cnd = $srch->addCondition('tpsr.pship_shipping_class_id', '=', intval($val));
                    break;
                case 'state':
                    //$cnd = $srch->addCondition('tpsr.pship_state', '=', intval($val));
                    
                    $cnd = $srch->addCondition('tpsr.pship_city', '=', '-1','AND');
                    $cnd = $srch->addCondition('tpsr.pship_state', '=', intval($val), 'AND');

                    
                    if ($include_everywhere) {
                        $cnd->attachCondition('tpsr.pship_state', '=', '-1', 'OR');
                    }
                    break;
                case 'city':
                    $cnd = $srch->addCondition('tpsr.pship_city', '=', intval($val));
                    break;   
        ...
        function getShippingClassById_before_city($product_id, $criteria = array(), $include_everywhere = TRUE)
        {
            ...
            //
            $srch->joinTable('tbl_shippingclass', 'LEFT JOIN', 'tpsr.pship_shipping_class_id=shipping_class.id', 'shipping_class');
            ...
            $srch->addMultipleFields(array('tpsr.*', 'tc.*', 'tsc.*', 'tsd.*', 'tpd.prod_ship_free', 'shipping_class.class_name', 'tstate.*'));
        ...
        function getProductShippingRates_X($product_id, $criteria = array())
        {
            $product_id = intval($product_id);
            if ($product_id > 0 != true)
                return array();
    ...
    function getProductShippingRates($product_id, $criteria = array(), $include_everywhere = TRUE) {
        $product_id = intval($product_id);
        if ($product_id > 0 != true)
            return array();
        $srch = new SearchBase('tbl_product_shipping_rates', 'tpsr');
        $srch->joinTable('tbl_prod_details', 'LEFT JOIN', 'tpsr.pship_prod_id=tpd.prod_id', 'tpd');
        $srch->joinTable('tbl_countries', 'LEFT JOIN', 'tpsr.pship_country=tc.country_id and tc.country_delete=0', 'tc');
        $srch->joinTable('tbl_states', 'LEFT JOIN', 'tpsr.pship_state=ts.state_id and ts.state_delete=0', 'ts');
        $srch->joinTable('tbl_shipping_companies', 'INNER JOIN', 'tpsr.pship_company=tsc.scompany_id and tsc.scompany_is_deleted=0', 'tsc');
        $srch->joinTable('tbl_shipping_durations', 'INNER JOIN', 'tpsr.pship_duration=tsd.sduration_id and tsd.sduration_is_deleted=0', 'tsd');
        $srch->joinTable('tbl_cities', 'LEFT JOIN', 'tpsr.pship_city=tci.city_id and tci.city_delete=0', 'tci');
        $srch->addCondition('tpsr.pship_prod_id', '=', intval($product_id));
       // $srch->addOrder('(`pship_country` = -1),country_name');
        $srch->addOrder('FIELD (`pship_state`,"-1") DESC,`pship_id`');
        $srch->addMultipleFields(array('tpsr.*', 'tc.*', 'tsc.*', 'tsd.*', 'tpd.prod_ship_free', 'ts.state_name','tci.city_name'));
...
//                        {
//                            $cnd->attachCondition('tpsr.pship_country', '=', '-1', 'OR');
//                            $cnd->attachCondition('tpsr.pship_country', '=', '0', 'OR');
//                        }
//                        break;
                case 'state':
                    $cnd = $srch->addCondition('tpsr.pship_city', '=', '-1','AND');
       ...
       // p($srch->getquery(),0);
        ...
        function getProductShippingRates_before_city($product_id, $criteria = array(), $include_everywhere = TRUE)
        {
            $product_id = intval($product_id);
            if ($product_id > 0 != true)
                return array();
            ...
            $srch->addCondition('tpsr.pship_prod_id', '=', intval($product_id));
            $srch->addOrder('(`pship_country` = -1),country_name');
            $srch->addMultipleFields(array('tpsr.*', 'tc.*', 'tsc.*', 'tsd.*', 'tpd.prod_ship_free', 'ts.state_name'));
...
//                        break;
        ...
        function getProductCommission($product_id)
        {
            ...
            $mydb = Syspage::getdb();
            //	$myquery="select * from tbl_subscription_merchant_package_orders where mporder_user_id = ".(int)$product['prod_added_by']." and date(NOW()) between mporder_subscription_start_date AND mporder_subscription_end_date order by mporder_id desc limit 1";
            //	$myquery="select * from tbl_subscription_merchant_package_orders where mporder_user_id = ".(int)$product['prod_added_by']." and mporder_subscription_end_date >= date(".date('Y-m-d').") order by mporder_id desc limit 1";

            $myquery = "SELECT *  FROM `tbl_subscription_merchant_package_orders` WHERE `mporder_user_id` = " . (int) $product['prod_added_by'] . " AND `mporder_subscription_end_date` >= '" . date('Y-m-d') . "' ORDER BY `mporder_id` DESC limit 1";


            //echo $myquery;
            $myrs = $this->db->query($myquery);
            //exit;
            if ($myrs->num_rows > 0)
            {
                $result = $this->db->fetch_all($myrs);
                //	echo '<pre>';
                //	print_r($result);
                //	exit;
                return $result[0]['mporder_merchantpack_commission'];
            }
            else
            {
        ...
        //Added on 23-08-2018
        public function joinWithTotalViewTable() {
            $this->joinTable('tbl_product_total_view', 'LEFT JOIN', 'tp.prod_id=tptv.prod_id', 'tptv');
        }
