...
class User {
	...
	function login($user_email_username, $user_password,$passwordAlreadyEncripted=false,$is_admin_login=false){
		...
		//$cnd=$srch->addCondition('LOWER(user_email)', '=', strtolower($user_email_username));
		//$cnd->attachCondition('LOWER(user_username)', '=', strtolower($user_email_username),'OR');
		...
		$sql='SELECT  * FROM `tbl_users` WHERE LOWER(user_email) = "'.strtolower($user_email_username).'" OR LOWER(user_username = "'.strtolower($user_email_username).'") AND `user_password` = "'.$user_password.'" ORDER BY user_id desc limit 0, 2000';
		//$sql = $srch->getQuery();
	//	echo $sql ;
		//exit;
	...
	//	exit;
		...
		if ( (!(strtolower($row['user_email']) === strtolower($user_email_username) || strtolower($row['user_username']) === strtolower($user_email_username))) || $row['user_password'] !== $user_password) {	
			$this->error = Utilities::getLabel('M_ERROR_INVALID_EMAIL_PASSWORD');
    ...
    function search($criteria,$not_in_ids=array()) {
		...
		$srch->addMultipleFields(array('tx.opr_order_id',"SUM(opr_qty-opr_refund_qty) as totQtys","SUM(opr_net_charged-opr_discount_total-opr_refund_amount) as totUserPurchase"));
	...
	function updateUser($data){
		...
		if(isset($data['ua_city']) && strlen($data['ua_city']) > 0){
			$values['user_city'] = $data['ua_city'];
		}
	...
	function updateUser_before_ci($data){
		$user_id = intval($data['user_id']);
		if($user_id < 1) return false;
	...
	function getUserAddresses($user_id,$pagesize=0){
		...
		$srch->joinTable('tbl_cities', 'LEFT OUTER JOIN', 'ci.city_id = ua.ua_city', 'ci');
		...
		$srch->doNotCalculateRecords();
		$srch->addMultipleFields(array('ua.*', 'c.country_name','c.country_id','c.country_code', 's.state_id','s.state_name','ci.city_id','ci.city_name'));
	...
	function getUserAddresses_before_ci($user_id,$pagesize=0){
		$user_id = intval($user_id);
		if($user_id < 1) return false;
		$srch = new SearchBase('tbl_user_address', 'ua');
		$srch->joinTable('tbl_countries', 'LEFT OUTER JOIN', 'c.country_id = ua.ua_country', 'c');
	...
	function getUserAddress($ua_id, $user_id){
		...
		$srch->doNotCalculateRecords();
		$srch->doNotLimitRecords();
		$srch->addMultipleFields(array('ua.*', 'c.country_name', 's.state_name','ci.city_name','ci.city_id'));
		//die($srch->getquery());
		$rs = $srch->getResultSet();
	...
	function getUserAddress_before_city($ua_id, $user_id){
		$ua_id = intval($ua_id);
		$user_id = intval($user_id);
		if($ua_id < 1 || $user_id < 1){
			return false;
	...
	function addUpdateAddress_before_city($data){
		$ua_id = intval($data['ua_id']);
		$user_id = intval($data['ua_user_id']);
		unset($data['ua_id']);
		unset($data['ua_user_id']);
	...
	//Ashish - Added function for supplier cancellation request on 22.06.2019
	
	function addOrderCancellationRequestSupplier($data){
		$user_id = intval($data['user_id']);
		$opr_id = intval($data['opr_id']);
		unset($data['user_id']);
		unset($data['opr_id']);
		...
		$assign_fields["cancellation_request_order"]=$order_detail["opr_id"];
		$assign_fields["cancellation_request_date"]=date('Y-m-d H:i:s');
		$assign_fields["cancellation_request_reason"]=-1;
		$assign_fields["cancellation_request_message"]=$data["message"];
		if($this->db->insert_from_array('tbl_order_cancel_requests', $assign_fields, true)){
			$this->cancellation_request=$this->db->insert_id();
			return true;
	...
	function approveRequest($id,$user_id,$action_by="U"){
		...
		if($this->db->update_from_array('tbl_prod_refund_requests', array('refund_request_status'=>2,'refund_request_updated_by'=>$user_id,'refund_request_action_by'=>$action_by), array('smt'=>'refund_id=?', 'vals'=>array($id))) && $this->db->insert_from_array('tbl_prod_refund_request_messages', array("refmsg_request"=>$id,"refmsg_from"=>$user_id,"refmsg_text"=>sprintf(Utilities::getLabel('L_Return_Request_Approved_By'),$approved_by),"refmsg_date"=>date('Y-m-d H:i:s'),"refmsg_from_type"=>$action_by), true)){
				...
				$coupon_discount_amt = $return_request['opr_discount_total'] * $return_request["refund_qty"]/$return_request["opr_qty"];
				$refund_amount = $refund_amount - $coupon_discount_amt;
	...
	function canAccessMyAccountPage($action){
		...
		$seller_pages = array(
							...
							"sales_print_order",
							"cancel_order",
							"seller_cancellation_request",	//Ashish - Added for seller cancellation requests.
							"ajax_product_list",
							"publications",
							"paused_publications",
							...
							"buyer_rating",
							"buyer_rating_details",
							"buyer_payment_options",
						);
